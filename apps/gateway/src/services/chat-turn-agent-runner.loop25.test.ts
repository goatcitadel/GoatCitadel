...
import { createExecuteToolCallForTest } from "./chat-turn-agent-runner-test-fixtures.js";

describe("ChatTurnAgentRunner loop 25 runtime preflight coverage", () => {
  it("infers fs.stat path arguments from explicit local project prompts", async () => {
    ...
    const result = await executeToolCall({
      input: turnInput({
        mode: "code",
        content: "Check metadata for `apps/gateway/src/services/chat-turn-agent-runner.ts` before editing.",
      }),
