...
describe("mesh route service", () => {
  it("delegates mesh reads and preserves optional arguments", () => {
    const deps = fakeDeps();
    const service = createMeshRouteService(createMeshRoutePort(deps));

    expect(service.getMeshSessionOwner("session-1")).toEqual({ sessionId: "session-1" });
    expect(service.getMeshStatus()).toEqual({ status: "online" });
    expect(service.listMeshLeases(5)).toEqual([{ leaseKey: "lease-1" }]);
    expect(service.listMeshNodes(6)).toEqual([{ nodeId: "node-1" }]);
    expect(service.listMeshReplicationEvents(7, "cursor-1")).toEqual([{ replicationId: "rep-1" }]);
    expect(service.listMeshReplicationOffsets(8)).toEqual([{ sourceNodeId: "node-1" }]);
    expect(service.listMeshSessionOwners(9)).toEqual([{ sessionId: "session-1" }]);
  ...
  it("emits realtime evidence for mesh mutations", () => {
    const deps = fakeDeps();
    const service = createMeshRouteService(createMeshRoutePort(deps));

    expect(service.acquireMeshLease({ leaseKey: "lease-1" } as never)).toEqual(lease());
    expect(service.claimMeshSessionOwner("session-1", { ownerNodeId: "node-2" } as never)).toEqual({
      ownerNodeId: "node-2",
      epoch: 3,
    });
    expect(service.ingestMeshReplicationEvent({ eventType: "chat.created" } as never)).toEqual({
      replicationId: "rep-1",
      sourceNodeId: "node-1",
      eventType: "chat.created",
      idempotencyKey: "idem-1",
    });
    expect(service.meshJoin({ nodeId: "node-3" } as never)).toEqual({
      node: { nodeId: "node-3", transport: "http", advertiseAddress: "http://node-3" },
    });
    expect(
      service.releaseMeshLease({ leaseKey: "lease-1", holderNodeId: "node-1", fencingToken: 11 } as never),
    ).toEqual({
      released: true,
    });
    expect(service.renewMeshLease({ leaseKey: "lease-1" } as never)).toEqual(lease());
