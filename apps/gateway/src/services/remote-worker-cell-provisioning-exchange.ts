import {
  ...
  normalizeRemoteWorkerCellPreparation, type RemoteWorkerCellPreparation,
  type RemoteWorkerCellProvisioningExchange,
} from "@goatcitadel/contracts";
import type { RemoteWorkerCellProvisioningAssignmentInput, RemoteWorkerCellPreparationAssignmentInput } from "@goatcitadel/storage";
...
export interface RemoteWorkerCellProvisioningExchangePort {
  exchange(input: RemoteWorkerCellProvisioningAssignmentInput & { readonly signal?: AbortSignal }):
    RemoteWorkerCellProvisioningExchange | Promise<RemoteWorkerCellProvisioningExchange>;
  prepare?(input: RemoteWorkerCellPreparationAssignmentInput & { readonly signal?: AbortSignal }):
...
export async function exchangeRemoteWorkerCellProvisioning(
  ...
  input.signal?.throwIfAborted();
  if (result.registryWorkspaceId !== input.registryWorkspaceId || result.assignmentId !== input.assignmentId ||
      result.assignmentGeneration !== input.assignmentGeneration || result.leaseRevision !== input.leaseRevision ||
...
export async function prepareRemoteWorkerCellProvisioning(
  ...
  if (exchange.registryWorkspaceId !== input.registryWorkspaceId || exchange.assignmentId !== input.assignmentId ||
      exchange.assignmentGeneration !== input.assignmentGeneration || exchange.leaseRevision !== input.leaseRevision ||
      exchange.plan.parentIdentityHex !== input.submission.parentIdentityHex || Date.parse(result.provisioningExpiresAt) <= Date.now()) {
