...
import { t, type I18nKey } from "./shell-command-explainer-i18n.js";
...
function nonFlagTokens(tokens: Tokens): string[] {
  return tokens.filter((token) => !isFlag(token));
}
...
function findCombinedShortFlag(tokens: Tokens, char: string): boolean {
  return tokens.some((token) => /^-[a-zA-Z]+$/.test(token) && token.includes(char));
}
...
const git: Handler = (tokens) => {
  const sub = tokens[1];
  if (sub === "push") {
    return gitPush(tokens);
  }
  if (sub === "reset") {
    return gitReset(tokens);
  }
...
const curlOrWget: Handler = (tokens) => {
  ...
  const url = after.find((token) => /^https?:\/\//.test(token)) ?? "(no URL)";
  const details: ShellExplanationDetail[] = [
...
const ssh: Handler = (tokens) => {
  const after = tokens.slice(1);
  const target = after.find((token) => !isFlag(token)) ?? "(no host)";
