...
describe("ChatTurnAgentRunner loop 28 coverage", () => {
  it("scores essential, suggested, project, memory, and access-filtered tools for code turns", async () => {
    ...
    const schema = await buildToolSchema(orchestrator, {
      ...
      webMode: "auto",
      content:
        "Use shell.exec, remember with consent that I prefer concise QA updates, inspect `apps/gateway/src/services/chat-turn-agent-runner.ts`, and search the web for the release note URL.",
      historyMessages: [],
    });
  ...
  it("blocks delegated non-code Prompt Lab local file calls before runtime invocation", async () => {
    ...
    const result = await executeToolCall({
      ...
      rawArgs: { path: "apps/gateway/src/services/chat-turn-agent-runner.ts", startLine: 1, endLine: 5 },
...
async function buildToolSchema(
  orchestrator: ChatTurnAgentRunner,
  input: {
    sessionId: string;
    mode: ChatTurnAgentRunnerInput["mode"];
    webMode: ChatTurnAgentRunnerInput["webMode"];
    content: string;
    historyMessages: ChatTurnAgentRunnerInput["historyMessages"];
    normalizationProfile?: ChatTurnAgentRunnerInput["normalizationProfile"];
