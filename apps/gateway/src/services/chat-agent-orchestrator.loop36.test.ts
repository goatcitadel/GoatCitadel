...
describe("ChatAgentOrchestrator loop36 tool-loop approval and failure handling", () => {
  ...
  it("records citations from successful model-requested file reads before final synthesis", async () => {
    ...
    const result = await orchestrator.run(
      turnInput(
        ...
        "Read apps/gateway/src/services/chat-agent-orchestrator.ts lines 10-12 and summarize them.",
      ),
    );
    ...
    expect(result.turnTrace.citations).toEqual([
      expect.objectContaining({
        sourceType: "file",
        title: "chat-agent-orchestrator.ts",
        url: "apps/gateway/src/services/chat-agent-orchestrator.ts",
