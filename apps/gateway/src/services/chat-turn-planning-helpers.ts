...
// Round-3 planner fan-out internals live in chat-planner-fanout.ts; the pieces
// consumers import from this module are re-exported so its surface stays stable.
export {
...
type ExecutionPlanDraftSteps = PreparedChatExecutionPlanResolution["executionPlanDraft"]["steps"];
...
export function coercePlannerExecutionPlanDraft(
  ...
  const steps: ExecutionPlanDraftSteps = templatePlan.steps.map((templateStep, index) => {
  ...
  if (expansionAllowed && rawSteps.length > templatePlan.steps.length) {
    steps.push(...buildDraftExpansionSteps(rawSteps, templatePlan));
  }
  const summary =
