...
describe("resolveChannelConfigTarget", () => {
  it("uses channel-specific target precedence", () => {
    expect(resolveChannelConfigTarget("signal", {
      defaultTarget: "fallback",
      defaultRecipient: "+15551234567",
    })).toBe("+15551234567");

    expect(resolveChannelConfigTarget("imessage", {
      defaultTarget: "fallback",
      defaultHandle: "+15557654321",
    })).toBe("+15557654321");

    expect(resolveChannelConfigTarget("nextcloud-talk", {
      defaultConversationId: "conversation-9",
      defaultTarget: "fallback",
    })).toBe("conversation-9");

    expect(resolveChannelConfigTarget("line", {
      defaultUserId: "U1234567890",
      defaultTarget: "fallback",
    })).toBe("fallback");

    expect(resolveChannelConfigTarget("zalo", {
  ...
  it("falls back to generic target fields for unknown channels", () => {
    expect(resolveChannelConfigTarget("custom-channel", {
  ...
  it("returns undefined when no configured target exists", () => {
    ...
    expect(resolveChannelConfigTarget("discord", {
      defaultChannelId: "   ",
    })).toBeUndefined();
  });
});
