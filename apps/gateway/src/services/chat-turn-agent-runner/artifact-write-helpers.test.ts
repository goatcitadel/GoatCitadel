...
describe("thread-grounded presentation artifacts", () => {
  ...
  it("builds a subject-specific deck from prior assistant research without visible Cowork copy", () => {
    ...
    expect((args.slides as Array<Record<string, unknown>>).every((slide) => slide.speakerNotes === undefined)).toBe(
      true,
    );
    expect(JSON.stringify(args)).not.toMatch(/Generated by GoatCitadel Cowork/i);
    expect(JSON.stringify(args)).not.toContain("Grounded in the prior conversation context.");
    expect(
      analyzePresentationContentQuality({
        args,
        content: "Put all that information into a presentation.",
...
- Compost unavoidable scraps after prevention efforts.`,
  ...
  it("preserves long slide bullets for renderer pagination without adding ellipses", () => {
    const longSentence =
    ...
    expect(bullet).toBe(longSentence);
    expect(bullet).not.toMatch(/(?:\.\.\.|…)$/u);
  });
