  ...
  it("delegates improvement route operations to the improvement service with original arguments", () => {
    ...
    const calls: Array<[keyof typeof improvement, () => unknown, unknown[]]> = [
      ...
      [
        ...
        () => service.requestImprovementActivation("candidate-10", { requesterId: "operator-a" }),
        ["candidate-10", { requesterId: "operator-a" }],
      ],
      ["getImprovementActivation", () => service.getImprovementActivation("activation-1"), ["activation-1"]],
      [
        "requestImprovementPauseApproval",
        () => service.pauseImprovementActivation("activation-2", { requesterId: "operator-b" }),
        ["activation-2", { requesterId: "operator-b" }],
      ],
      [
        "requestImprovementRollbackApproval",
        () => service.rollbackImprovementActivation("activation-3", { requesterId: "operator-c" }),
        ["activation-3", { requesterId: "operator-c" }],
      ],
