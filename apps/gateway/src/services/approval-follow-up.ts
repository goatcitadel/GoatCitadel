import type { ApprovalEffectRecord, ApprovalRequest, ApprovalResolutionOutcome } from "@goatcitadel/contracts";
...
export async function buildApprovalResolveResult(
  ...
  approval = await withCanonicalApprovalOutcome(storage, withApprovalFollowUp(approval, effects));
  return {
    approval,
    effects,
