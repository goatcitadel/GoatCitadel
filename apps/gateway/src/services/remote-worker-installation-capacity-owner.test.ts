...
import {
  ...
  type RemoteWorkerInstallationReservation,
} from "./remote-worker-installation-capacity-owner.js";
import {
  createRemoteWorkerExecutionOwners,
  type RemoteWorkerExecutionOwnersDependencies,
...
function fixture() {
  ...
  const window = { verify: vi.fn(async () => {}) },
    policy = { verify: vi.fn(async () => {}) };
  const storage = {
    remoteWorkerRuntimeInstalls: { validatePoolCaptureForAssignment: validate },
  } as unknown as ConstructorParameters<typeof RemoteWorkerInstallationCapacityOwner>[0];
...
describe("installation reservation lifetime", () => {
  it("checks before finish and retains the assignment until helper shutdown joins", async () => {
    const f = fixture();
    await expect(
      f.owner.run(f.input, async (reservation) => {
        ...
        return reservation.finish(async () => {
          ...
          await expect(
            f.owner.run({ ...f.input, window: { verify: async () => {} } }, async () => "overlap"),
          ).rejects.toThrow(/active reservation/u);
  ...
  it.each(["repeat", "challenge", "cancel", "join-failure"])(
    "poisons %s during terminal shutdown even if swallowed",
    async (mode) => {
      const f = fixture();
      await expect(
        f.owner.run(f.input, async (reservation) => {
  ...
  it("drains an unawaited helper join before releasing the assignment", async () => {
    ...
    const run = f.owner.run(f.input, async (reservation) => {
      ...
      void reservation
        ...
        .catch(() => {
          /* Intentionally detached; the owner rejection is asserted below. */
        });
      await joining;
    });
    void run.catch(() => {
    ...
    await expect(f.owner.run({ ...f.input, window: { verify: async () => {} } }, async () => {})).rejects.toThrow(
      /active reservation/u,
    );
    release();
  ...
  it("retains exact binding, rechecks each challenge and closes leaked handles", async () => {
    const f = fixture();
    let leaked: RemoteWorkerInstallationReservation | undefined;
    await expect(
      f.owner.run(f.input, async (reservation) => {
        leaked = reservation;
        await reservation.verify(f.challenge());
        await reservation.verify(f.challenge(2));
        return "done";
      }),
    ).resolves.toBe("done");
    expect(f.validate).toHaveBeenCalledTimes(4);
    expect(f.window.verify).toHaveBeenCalledTimes(8);
    expect(f.policy.verify).toHaveBeenCalledTimes(8);
  ...
  it.each(["window", "policy", "storage", "baseline", "binding", "cancel"])(
    "refuses changed authority during a check: %s",
    async (mode) => {
      const f = fixture();
      await expect(
        f.owner.run(f.input, async (reservation) => {
  ...
  it.each(["ordinal", "replay", "foreign", "trailing"])(
    "rejects invalid challenge without another canonical read: %s",
    async (mode) => {
      const f = fixture();
      await expect(
        f.owner.run(f.input, async (reservation) => {
  ...
  it("freezes command and bytes before asynchronous checks", async () => {
    const f = fixture(),
      original = f.challenge();
    f.window.verify.mockImplementationOnce(async () => {
      f.binding.installationNonce = "88".repeat(32);
    });
    await f.owner.run(f.input, async (reservation) => {
      await reservation.verify(original);
    });
  ...
  it("does not accept an operation with no completed native challenge", async () => {
    const f = fixture();
    await expect(f.owner.run(f.input, async () => "unused")).rejects.toThrow(/without completed/u);
  });
  it("refuses concurrent assignment lifetimes and releases after failure", async () => {
    const f = fixture();
    await expect(
      f.owner.run(f.input, async () => {
        await expect(
          f.owner.run({ ...f.input, window: { verify: async () => {} } }, async () => "overlap"),
        ).rejects.toThrow(/active reservation/u);
    ...
    await f.owner.run({ ...f.input, window: { verify: async () => {} } }, async (reservation) => {
      await reservation.verify(f.challenge());
    });
  });
  it.each([0, 60001])("refuses wall limit %i before external checks", async (wallLimitMs) => {
    const f = fixture();
    await expect(f.owner.run({ ...f.input, wallLimitMs }, async () => "bad")).rejects.toThrow();
    expect(f.validate).not.toHaveBeenCalled();
    expect(f.window.verify).not.toHaveBeenCalled();
  });
  it("expires its live signal without extending the bounded window", async () => {
    const f = fixture();
    await expect(
      f.owner.run({ ...f.input, wallLimitMs: 50 }, async (reservation) => {
  ...
  it("drains an unawaited check without reporting a successful operation", async () => {
    ...
    const run = f.owner.run(f.input, async (reservation) => {
      ...
      const check = reservation.verify(f.challenge());
      void check.catch(() => {
        /* Preserve the intentionally detached check; assert the owner rejection below. */
      });
  ...
  it.each([true, false])("composes the reservation owner with policy present=%s", async (present) => {
    ...
    const dependencies = {
      ...
      approvals: {},
      llm,
      completionHost: { llmService: llm },
      artifactRoot: "unused-install-reservation",
      ...(present ? { nativeInstallationPolicy: f.policy } : {}),
    ...
    const operation = vi.fn(async (reservation: RemoteWorkerInstallationReservation) => {
      await reservation.verify(f.challenge());
      return "done";
    });
