...
async function createMeshCoordinatorFixture() {
  ...
  const [schema] = await resolveMeshChatToolSchemas(catalog.deps, {
    workspaceId: catalog.workspaceId,
    entries: [catalog.entry],
  });
  const binding = { schema: schema!, executionProfileSha256: "9".repeat(64) };
  const append = vi.fn();
  const dispatch = vi.fn<NonNullable<ToolInvocationCoordinatorHost["dispatchMeshCapabilityInvocation"]>>(
    async (_input, options) => {
      ...
      return {
        ...
        receipt: {
          ...
          nodeId: catalog.binding.nodeId,
          activationId: catalog.binding.activationId,
          activationRevision: catalog.binding.activationRevision,
  ...
  const host = createHost({
    isValidToolName: (name) => /^[a-zA-Z0-9][a-zA-Z0-9._-]{0,254}$/.test(name),
    resolveMeshChatToolBinding: vi.fn(async () => binding),
    dispatchMeshCapabilityInvocation: dispatch,
  });
  const request = createToolRequest({
    toolName: catalog.capabilityId,
    workspaceId: catalog.workspaceId,
    turnId: "turn",
...
describe("mesh tools in the canonical invocation coordinator", () => {
  it("runs hooks and mapped policy before the generation-fenced mesh owner", async () => {
    ...
    expect(f.host.policyEngine.invoke).toHaveBeenCalledWith(
      { ...f.request, externalRuntime: true },
      {
  ...
  it.each(["blocked", "approval_required"] as const)(
    "retains %s policy without creating a mesh intent",
    async (outcome) => {
      const f = await createMeshCoordinatorFixture();
      vi.mocked(f.host.policyEngine.invoke).mockResolvedValue({
        outcome,
        approvalId: outcome === "approval_required" ? "approval-1" : undefined,
        policyReason: outcome,
        auditEventId: "audit",
      });
      const result = await f.coordinator.invokeTool(f.request);
      ...
      if (outcome === "approval_required")
        expect(f.host.primeToolApprovalLifecycle).toHaveBeenCalledWith("approval-1", f.request);
    },
  );
  ...
  it("rejects hook target rewrites and plugin replacement of an admitted mesh tool", async () => {
    ...
    expect(await f.coordinator.invokeTool(f.request)).toMatchObject({
      outcome: "blocked",
    ...
    overrides.registerOverrideClaim({
  ...
  it("refuses an inline approval replay and preserves the canonical approval worker owner", async () => {
    ...
    vi.mocked(f.host.policyEngine.invoke).mockResolvedValue({
      ...
      audit: {
        ...
        approvalId: "reviewed-approval",
      },
    });
    const result = await f.coordinator.invokeTool({
      ...f.request,
...
describe("ToolInvocationCoordinatorService", () => {
  it.each(["before-prepare", "before-execute"])("rejects an approved tool owner replaced %s", async (phase) => {
    ...
    const handler = vi.fn(
      async (): Promise<ToolInvokeResult> => ({
        outcome: "executed",
        policyReason: "replaced",
        auditEventId: "changed",
      }),
    );
    const replace = () => {
    ...
    if (phase === "before-prepare") {
      replace();
      await expect(
        coordinator.prepareApprovedBuiltinBeforeExecute(request, { invocationId: "resume", runtimeOwner }),
      ).rejects.toThrow("runtime owner changed");
    } else {
      const check = (await coordinator.prepareApprovedBuiltinBeforeExecute(request, {
        invocationId: "resume",
        runtimeOwner,
      }))!;
  ...
  it("rechecks turn consent when denial wins during asynchronous policy evaluation", async () => {
    let denied = false;
    const starts = vi.fn();
    const coordinator = new ToolInvocationCoordinatorService(
      createHost({
        assertChatToolDispatchAllowed: async () => {
          if (denied) throw new Error("turn denied");
        },
        policyEngine: {
          evaluateAccess: vi.fn(() => ({ allowed: true, requiresApproval: false, reasonCodes: [] })),
          invoke: vi.fn(async (_request: ToolInvokeRequest, options?: { beforeExecute?: () => Promise<void> }) => {
            await Promise.resolve();
            denied = true;
            await options?.beforeExecute?.();
            starts();
            return { outcome: "executed" as const, policyReason: "allowed", auditEventId: "should-not-start" };
          }),
        },
      }),
    );
    ...
    ).rejects.toThrow("turn denied");
    expect(starts).not.toHaveBeenCalled();
  });

  it.each(["execute", "approval", "stale", "receipt_failed"] as const)(
    "records the builtin execution boundary only after policy and execution fences: %s",
    async (mode) => {
      const events: string[] = [];
        ...
        createHost({
          policyEngine: {
            ...
            invoke: vi.fn(
              async (
                _request: ToolInvokeRequest,
                options?: { beforeExecute?: () => Promise<void> },
              ): Promise<ToolInvokeResult> => {
                if (mode === "approval")
                  return {
                    outcome: "approval_required",
                    approvalId: "approval-boundary",
                    policyReason: "operator review",
                    auditEventId: "audit-boundary",
                  };
                await options?.beforeExecute?.();
                events.push("read");
      ...
      const result = coordinator.invokeTool(createToolRequest({ toolName: "fs.read", args: { path: "note.txt" } }), {
        executionFence: async () => {
          events.push("lease");
          if (mode === "stale") throw boundaryFailure;
        },
        beforeBuiltinExecute: async () => {
          events.push("receipt");
          if (mode === "receipt_failed") throw boundaryFailure;
        },
      });
  ...
  it("keeps auxiliary after-hook dispatch distinct from a pre-executor approval", async () => {
    ...
    expect(enqueueAfterHooks).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        beforeExternalDispatch: expect.any(Function),
      }),
    );
  });
    ...
    async (scenario) => {
      ...
      try {
        ...
        if (scenario === "allowed") {
          expect(result).toMatchObject({
            outcome: "executed",
            result: { ok: true, output: { payload: "native MCP response" } },
          });
          expect(sequence).toEqual(["fence", "receipt", "transport"]);
          expect(host.invokeMcpRuntimeTool).toHaveBeenCalledTimes(1);
          expect(
            storage.toolAccessDecisions.countToolCallsInLastHour("mcp.invoke", request.agentId, request.sessionId),
          ).toBe(1);
          expect(
            storage.toolAccessDecisions.countToolCallsInLastHour(
              "mcp.srv-1.tool.echo",
              request.agentId,
              request.sessionId,
            ),
          ...
          expect(host.invokeMcpRuntimeTool).toHaveBeenCalledTimes(2);
          config.tools.deny = [];
          storage.citadels.addWard({
            citadelId: "personal",
            name: "Native MCP fixture deny",
            actionPattern: "mcp.srv-1.tool.echo",
            effect: "deny",
          });
          ...
          expect(result.outcome).toBe(
            scenario === "approval" ? "approval_required" : scenario.endsWith("denied") ? "blocked" : "executed",
          );
  ...
  it.each(["allowed", "denied", "approval", "dry-run", "mode-drift", "profile-drift"] as const)(
    ...
    async (scenario) => {
      ...
      const policyEngine = new ToolPolicyEngine(config, createSqliteAsyncStorage(storage));
      const policyInvoke = vi.spyOn(policyEngine, "invoke");
      const policyBinding = createMcpToolPolicyBinding({
        canonicalName: "mcp.srv-1.tool.echo",
        serverId: "srv-1",
        nativeToolName: "tool.echo",
      });
      ...
      const sequence: string[] = [];
      const transport = vi.fn<NonNullable<ToolInvocationCoordinatorHost["requesterScopedMcpDispatch"]>["invoke"]>(
        async (_input, options) => {
      ...
      const host = createHost({
        policyEngine,
        resolveNativeMcpChatToolBinding: resolveBinding,
      ...
      const request = createToolRequest({
        ...
        toolRunId: "tool-run",
        workspaceId: "workspace-1",
        args: { serverId: "native data", toolName: "native data", value: "hello" },
        ...(scenario === "dry-run" ? { dryRun: true } : {}),
      });
      const coordinator = new ToolInvocationCoordinatorService(host);
      try {
        ...
        expect(policyInvoke).toHaveBeenCalledExactlyOnceWith(
          { ...request, externalRuntime: true },
          { mcpToolBinding: policyBinding },
        );
        expect(host.invokeMcpRuntimeTool).not.toHaveBeenCalled();
        if (scenario === "allowed") {
          ...
          expect(
            storage.toolAccessDecisions.countToolCallsInLastHour(request.toolName, request.agentId, request.sessionId),
          ).toBe(1);
        } else {
          expect(transport).not.toHaveBeenCalled();
          expect(sequence).toEqual([]);
          expect(result.outcome).toBe(
            scenario === "approval" ? "approval_required" : scenario === "denied" ? "blocked" : "executed",
          );
          if (scenario === "approval") {
            expect(storage.pendingApprovalActions.find(result.approvalId!)?.request).toMatchObject({
              ...request,
  ...
  it("rejects unbound native calls before hooks, policy, or transport", async () => {
    const host = createHost();
    const result = await new ToolInvocationCoordinatorService(host).invokeTool(
      createToolRequest({
        toolName: "mcp.srv-1.tool.echo",
        turnId: "turn",
        toolRunId: "tool-run",
  ...
  it.each(
    ["first-use", "blocked", "not-allowlisted"].flatMap((gate) =>
      ["wrapper", "native"].map((form) => [gate, form] as const),
    ),
    ...
    const host = createHost({
      requireMcpServer: vi.fn(async () => server),
      isMcpToolApproved: vi.fn(async () => false),
      requesterScopedMcpDispatch: { invoke: transport },
      resolveNativeMcpChatToolBinding: vi.fn(async () => ({
        serverId: "srv-1",
        nativeToolName: "tool.echo",
    ...
    const result = await new ToolInvocationCoordinatorService(host).invokeTool(
      createToolRequest({
        toolName: form === "native" ? "mcp.srv-1.tool.echo" : "mcp.invoke",
        turnId: "turn",
        toolRunId: "tool-run",
  ...
  it.each(["first-use", "blocked", "mode-drift", "target-drift", "plugin-drift"] as const)(
    "keeps approved native MCP replay inside its reviewed owner (%s)",
    async (scenario) => {
      const server = createMcpServer({
        connectionMode: scenario === "mode-drift" ? "static" : "requester_scoped",
        policy: {
      ...
      const host = createHost({
        ...
        isMcpToolApproved: vi.fn(async () => false),
        requesterScopedMcpDispatch: { invoke: transport },
        ...(scenario === "plugin-drift"
          ? {
              pluginToolOverrideService: {
                resolveActiveHandler: vi.fn(() =>
                  vi.fn(async () => ({
                    outcome: "executed" as const,
                    policyReason: "unexpected plugin",
                    auditEventId: "plugin",
                  })),
                ),
              },
      ...
      const markStarted = vi.fn();
      const call = new ToolInvocationCoordinatorService(host).invokeApprovedMcpRuntime(
        {
          serverId: "srv-1",
          toolName: "tool.echo",
          agentId: "assistant",
        ...
        markStarted,
        { nativeCanonicalToolName: scenario === "target-drift" ? "mcp.other.tool.echo" : "mcp.srv-1.tool.echo" },
      );
  ...
  it.each(["execution-fence", "durable-receipt"] as const)(
    ...
    async (boundary) => {
      const host = createHost();
      const rejectBoundary = vi.fn(async () => {
        throw new Error("worker authority lost");
      });
      ...
      await expect(
        new ToolInvocationCoordinatorService(host).invokeTool(request, {
  ...
  it("retains unknown-after-send truth for a directly allowed MCP call", async () => {
    ...
    const result = await new ToolInvocationCoordinatorService(host).invokeTool(
      createToolRequest({
        toolName: "mcp.invoke",
        args: { serverId: "srv-1", toolName: "tool.echo", arguments: {} },
      }),
    );
    expect(result).toMatchObject({
      outcome: "executed",
      policyReason: expect.stringContaining("manual reconciliation"),
  ...
  it("blocks a generic MCP approval replay outside the canonical effect owner", async () => {
    ...
    expect(result).toMatchObject({
      outcome: "blocked",
      result: { approvalId: "approval-mcp", executionOwner: "approval_effect" },
    });
    expect(host.invokeMcpRuntimeTool).not.toHaveBeenCalled();
  });
  ...
  it("keeps protected approval action delivery on the native policy executor despite an active channel override", async () => {
    ...
    expect(policyInvoke).toHaveBeenCalledTimes(1);
    expect(policyInvoke).toHaveBeenCalledWith(
      expect.objectContaining({ toolName: "channel.send" }),
      expect.objectContaining({ beforeExecute: expect.any(Function) }),
    );
...
describe("ToolInvocationCoordinatorService requester-scoped MCP seam (HX-415)", () => {
  ...
  it.each(["mcp", "tool", "approved"] as const)(
    "threads branded context and effect fences through the %s entry point",
    async (entry) => {
      const sequence: string[] = [];
