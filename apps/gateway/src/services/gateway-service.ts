...
export class GatewayService {
  ...
  public async runChatDelegation(
    ...
    this.taskLifecycleService.appendTaskActivity(task.taskId, {
    ...
    const executeDelegationStep = async (step: NormalizedDelegationStep): Promise<DelegationStepExecutionResult> => {
      ...
      this.taskLifecycleService.registerTaskSubagent(task.taskId, {
        agentSessionId,
        agentName: step.role,
      });
      ...
      try {
        ...
        this.taskLifecycleService.updateTaskSubagent(agentSessionId, {
          status: failed ? "failed" : "completed",
          endedAt: finishedAt,
        });
        this.taskLifecycleService.appendTaskActivity(task.taskId, {
        ...
        if (!failed) {
          this.taskLifecycleService.appendTaskDeliverable(task.taskId, {
    ...
    if (completedSteps > 0 && status === "completed") {
      this.taskLifecycleService.updateTask(task.taskId, {
        status: "review",
      });
