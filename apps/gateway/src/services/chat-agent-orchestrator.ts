...
function buildPromptLabKnownMinimalTestSpecFallback(input: {
  ...
  if (looksLikePromptLabJudgeDefaultsMinimalTestPrompt(userTask)) {
    ...
    return [
      `- Target test file or suite: \`${testPath}\``,
      '- Setup: Add one scoring-path test beside the existing prompt-pack service scoring tests. Use two explicit fixtures: `frozenBaselineJudgeDefaults = { providerId: "openai", model: "gpt-5.4" }` and `expandedPackJudgeDefaults = { providerId: "openai-codex", model: "gpt-5.5" }`. Seed an expanded-pack `PromptPackRecord`, one completed run whose evaluated runner is `moonshot/kimi-k2.6`, and a complete current judge JSON payload with `routingScore`, `honestyScore`, `handoffScore`, `robustnessScore`, `usabilityScore`, and `rationale` so the scorer cannot pass through fallback parsing.',
      ...
      '- Assert: The actual judge `createChatCompletion(...)` request and persisted auto-score row both use `{ providerId: "openai-codex", model: "gpt-5.5" }`; separately assert the evaluated run remains `{ providerId: "moonshot", model: "kimi-k2.6" }` and the frozen-baseline defaults remain only a comparison fixture.',
      "- Failure signature: Fail if the scoring path sends the judge to `gpt-5.4`, sends it to the runner model `kimi-k2.6`, needs explicit input overrides to get `gpt-5.5`, or accepts an incomplete mock judge JSON response.",
      "",
      "```ts",
      ...
      '  const run = { ...createRun("run-expanded-defaults", "completed", nowIso), testId: test.testId, providerId: "moonshot", model: "kimi-k2.6", responseText: "Answered.", trace: createTrace("sess-expanded-defaults") };',
      ...
      '  expect({ providerId: run.providerId, model: run.model }).toEqual({ providerId: "moonshot", model: "kimi-k2.6" });',
