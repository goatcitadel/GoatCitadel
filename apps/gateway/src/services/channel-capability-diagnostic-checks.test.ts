...
describe("buildChannelCapabilityDiagnosticChecks", () => {
  it("reports outbound-only channels explicitly", () => {
    const checks = buildChannelCapabilityDiagnosticChecks(createCapabilities({
      inboundModes: ["none"],
    }));
    ...
    expect(checks).toEqual(expect.arrayContaining([
      ...
      },
      {
        key: "runtime_posture",
        status: "warn",
        message: "Runtime posture: Outbound-only webhook delivery; no inbound runtime or pairing session is available.",
      },
      {
        key: "runtime_policy",
        status: "pass",
  ...
  it("reports gateway inbound modes and enabled runtime policy flags", () => {
    ...
    expect(checks).toEqual(expect.arrayContaining([
      ...
      },
      {
        key: "runtime_posture",
        status: "pass",
        message: "Runtime posture: Persistent gateway runtime keeps Discord inbound messages, typing, and presence synchronized in-process.",
      },
      {
        key: "runtime_policy",
        status: "pass",
        message: "Runtime policy gates: allowlist, mentionGating, pairing, presence, typing.",
      },
      {
        key: "setup_ready",
        status: "pass",
  ...
  it("surfaces setup follow-up diagnostics and support notes", () => {
    const checks = buildChannelCapabilityDiagnosticChecks(createCapabilities({
      setupReady: false,
      setupDiagnostics: [
        "Missing sandbox channel.",
        "Webhook test not yet completed.",
      ],
      supportNotes: [
        "Gateway mode supports richer inbound routing than webhook fallback.",
      ],
    }));
