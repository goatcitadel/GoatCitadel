...
function toolTurn(sessionId: string, content: string): ChatTurnAgentRunnerInput {
  return {
    sessionId,
    turnId: randomUUID(),
...
describe("ChatTurnAgentRunner P0-B answer-recovery ladder", () => {
  ...
  it("records distinct degraded.reason for a deterministic vs a model-synthesized empty-output recovery", async () => {
    ...
    const recoveredOrchestrator = new ChatTurnAgentRunner({
      storage: createMockStorage() as never,
    ...
    const deterministicOrchestrator = new ChatTurnAgentRunner({
      storage: createMockStorage() as never,
      listToolCatalog: () => createToolCatalog(["memory.search"]),
    ...
    expect(recovered.turnTrace.completion?.degraded?.reason).not.toBe(floored.turnTrace.completion?.degraded?.reason);
  });
