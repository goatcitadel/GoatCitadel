...
export const devDiagnosticsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/api/v1/dev/diagnostics/stream", async (request, reply) => {
    ...
    try {
      ...
      raw.flushHeaders?.();
      const wroteConnected = await writeSseChunk(raw, ": connected\n\n", controller.signal);
      if (!wroteConnected) {
        throw new Error("dev_diagnostics_sse_initial_write_failed");
      }
      ...
      for (const item of replay) {
        const wroteReplayItem = await send(item);
        if (!wroteReplayItem) {
          throw new Error("dev_diagnostics_sse_replay_write_failed");
        }
      }
