...
describe("dev diagnostics stream route", () => {
  ...
  it("releases the stream slot when the initial SSE write fails", async () => {
    process.env.GOATCITADEL_DEV_DIAGNOSTICS_SSE_MAX_CONNECTIONS_PER_IP = "1";
    const handler = await getStreamHandler({
      devDiagnostics: {
        isDevDiagnosticsEnabled: () => true,
    ...
    const failingRaw = new FakeRaw();
    vi.spyOn(failingRaw, "write").mockImplementation(() => {
      throw new Error("socket write failed");
    });

    await expect(
      handler({ query: { replay: "1" }, raw: new EventEmitter(), ip: "127.0.0.1" }, createReply(failingRaw)),
    ).rejects.toThrow("socket write failed");

    const nextRaw = new FakeRaw();
    await handler({ query: { replay: "1" }, raw: new EventEmitter(), ip: "127.0.0.1" }, createReply(nextRaw));

    expect(failingRaw.end).toHaveBeenCalledTimes(1);
    expect(nextRaw.statusCode).toBe(200);
    nextRaw.emit("close");
  });
});
