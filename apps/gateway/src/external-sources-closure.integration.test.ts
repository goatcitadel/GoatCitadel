...
// See external-sources.integration.test.ts: external-source registration is gated
// on the Windows-only workspace path bridge (path.win32), so the windows_native
// resolve endpoint 500s on a POSIX host. This full-stack closure flow only runs on
// Windows; domain logic keeps POSIX coverage in the unit/component suites.
describe.skipIf(process.platform !== "win32")(
  "HX-407 C4 external source closure composition",
  { timeout: 180_000 },
  () => {
    afterEach(async () => {
    ...
    it("drives register→scan→plan→apply→attach→send→knowledge-approval→recovered snapshot end-to-end without the proof gate", async () => {
      ...
      try {
        ...
        const staleIncarnation = await app.inject({
          ...
          payload: {
            ...attachBody,
            itemId: applied.items[1]!.itemId,
            expectedSessionIncarnationId: "incarnation-stale",
          },
        });
