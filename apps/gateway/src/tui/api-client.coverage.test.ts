...
describe("TuiApiClient coverage", () => {
  ...
  it("invokes broad API surface and stream handling", async () => {
    ...
    const specialArgs: Record<string, unknown[]> = {
      ...
      getIntegrationConnection: ["conn-1"],
      createIntegrationConnection: [
        { catalogId: "x", label: "Coverage", enabled: true, status: "connected", config: {} },
      ],
      updateIntegrationConnection: ["conn-1", { status: "connected", expectedRevision: "a".repeat(64) }],
      deleteIntegrationConnection: ["conn-1", "a".repeat(64)],
      meshStatus: [],
      meshNodes: [],
      meshLeases: [],
      meshOwners: [],
      ...
      listCronJobs: [],
      startCronJob: ["job-1", 1],
      pauseCronJob: ["job-1", 1],
      runCronJob: ["job-1"],
      deleteCronJob: ["job-1", 1],
