...
/** The path flavor the bridge serves on this host. */
const HOST_PATH_FLAVOR = process.platform === "win32" ? "windows_native" : "posix";
...
// External-source registration is gated on the workspace path bridge, which
// resolves the host's own path flavor: `windows_native` on Windows, `posix`
// elsewhere. The flow is identical either way, so this full-stack suite runs on
// every host rather than being skipped off Windows.
    ...
    try {
      ...
      const verifiedResponse = await app.inject({
        ...
        payload: {
          ...
          inputFlavor: HOST_PATH_FLAVOR,
          targetFlavor: HOST_PATH_FLAVOR,
