...
import { buildAddonProductPosture } from "./addon-product-posture";
...
export function AddonsSection(props: SettingsSectionProps) {
  ...
  const loadPacks = useCallback(async () => {
    if (!packsRequested)
      return {
        issues: [],
        capabilityPacks: [] as CapabilityPackManifest[],
        stagedPacks: [] as CapabilityPackStagedRecord[],
      };
    ...
    return {
      issues: nativeLoadIssues([packs, staged]),
      capabilityPacks: packs.data.items,
      stagedPacks: staged.data.items,
    };
  }, [packsRequested]);
  ...
  const {
    ...
    data: packData,
    reload: reloadPacks,
  } = useAsyncLoad(loadPacks, [loadPacks]);
  const data = useMemo(
    () =>
      addonData
        ? {
            ...addonData,
            capabilityPacks: packData?.capabilityPacks ?? [],
            stagedPacks: packData?.stagedPacks ?? [],
            issues: [...addonData.issues, ...(packData?.issues ?? [])],
          }
        : null,
    [addonData, packData],
  );
  const reload = async () => {
    await Promise.all([reloadAddons(), packsRequested ? reloadPacks() : Promise.resolve()]);
  };
  const [notice, setNotice] = useState<Notice | null>(null);
  const [selectedAddonId, setSelectedAddonId] = useSessionViewState("addons:selected", "");
  const selectionRef = useRef(selectedAddonId);
  selectionRef.current = selectedAddonId;
  ...
  const packSelectionRef = useRef(selectedPackId);
  packSelectionRef.current = selectedPackId;
  ...
  const localDraft = useSessionDraft("portable-pack:" + props.activeWorkspaceId + ":import", "", undefined, {
    label: "Portable pack",
    active: panel === "portable",
    onSave: () => stageLocalPack(),
  });
  const { value: localPackText, setValue: setLocalPackText } = localDraft;
  const localSourceRef = useRef(localPackText);
  localSourceRef.current = localPackText;
  ...
  const selectedPack = data?.capabilityPacks?.find((item) => item.packId === selectedPackId) ?? null;
  ...
  const stageLocalPack = async (): Promise<boolean> => {
    ...
    if (!localPackPreview.data || reviewedLocalText !== localPackText) {
      setNotice({ tone: "warning", message: "Preview the current manifest before staging it." });
      return false;
    }
    ...
    try {
      ...
      if (localSourceRef.current === submitted)
  ...
  return (
    <SettingsSectionShell loading={loading} error={error} onRetry={reload}>
      ...
      <SettingsStack>
        ...
        <p className="mc-next-settings-experimental-note">
          Experimental local extensions · operator-reviewed installation
        </p>
        {panel === "portable" ? (
          <FocusedDetail title="Portable pack" onClose={closePanel}>
            <SettingsFieldGrid>
            ...
            <SettingsButtonRow>
              ...
              <NativeButton
                variant="default"
                disabled={
                  staging || localPackPreview.loading || !localPackPreview.data || reviewedLocalText !== localPackText
                }
          ...
          <FocusedDetail title={selectedPack?.name ?? "Capability pack unavailable"} onClose={closePanel}>
            <SettingsStack>
              <p>{selectedPack?.description}</p>
              <p>
                {selectedPack?.trustTier} · {selectedPack?.version}
              </p>
              {selectedPack && packPreview.data ? (
                <PackExecutionPanel
                ...
                <p role="status">
                  {packPreview.error ? "Pack preview unavailable: " + packPreview.error : "Loading pack preview..."}
                </p>
              )}
              <NativeDisclosureCard
                id="pack-staging-and-export"
                title="Staging and portable manifest"
                subtitle="Review installation warnings, advisory policy defaults, staging evidence, and export."
              >
                {selectedPack ? (
                  <>
                    <p>{selectedPack.description}</p>
          ...
          <>
            <SettingsButtonRow>
              ...
              >
                Import pack{localDraft.isDirty ? " · Unsaved" : ""}
              </NativeButton>
            ...
            <SettingsFilterBar
              options={[
                ...
                { id: "packs", label: "Capability packs" },
              ]}
              value={view}
              onChange={(next) => {
                setView(next as "addons" | "packs");
                if (next === "packs") setPacksRequested(true);
                setPanel(null);
              }}
            />
            {view === "addons" && data ? (
              <NativeCard
                ...
                stats={[
                  {
                    label: "Catalog",
                    value: data.issues.some((issue) => issue.label === "Add-on catalog")
                      ? "Unavailable"
                  ...
                  {
                    label: "Installed",
                    value: data.issues.some((issue) => issue.label === "Installed add-ons")
                      ? "Unavailable"
                      : Array.isArray(data.installed)
                        ? String(data.installed.length)
                        : "Unavailable",
                  },
                ]}
              >
              ...
              <>
                ...
                {packsLoading ? (
                  <p role="status">Loading capability packs...</p>
                ) : (
                  <NativeCard
                    ...
                    stats={[
                      {
                        ...
                        value: !packData
                          ? "Loading"
                          : packData.issues.some((issue) => issue.label === "Capability packs")
                            ? "Unavailable"
                            : Array.isArray(packData.capabilityPacks)
                              ? String(packData.capabilityPacks.length)
                              : "Unavailable",
                      },
                      {
                        label: "Staged",
                        value: !packData
                          ? "Loading"
                          : packData.issues.some((issue) => issue.label === "Staged capability packs")
                            ? "Unavailable"
                            : Array.isArray(packData.stagedPacks)
                              ? String(packData.stagedPacks.length)
                              : "Unavailable",
                      },
                      { label: "Selected", value: selectedPack?.trustTier ?? "None selected" },
                    ]}
                  >
                    <NativeSelectableList
                ...
                <NativeDisclosureCard id="pack-staged-evidence" title="Staged packs and review evidence">
                  <SettingsActionList
      ...
      <DetailInspector
        open={panel === "addon"}
        title={selectedAddon?.label ?? "Add-on unavailable"}
        onClose={closePanel}
      >
        {status.loading ? <p role="status">Loading add-on status...</p> : null}
        {status.error ? (
          <SettingsNotice notice={{ tone: "error", message: "Add-on status unavailable: " + status.error }} />
        ) : null}
          ...
          <>
            ...
            <SettingsButtonRow>
              ...
              <NativeButton
                ...
                onClick={() =>
                  void runAddonAction(() => stopAddon(selectedAddon.addonId), `${selectedAddon.label} stop requested.`)
                }
              >
                <Square size={16} />
                Stop
            ...
            <NativeDisclosureCard id="addon-diagnostics" title="Install commands and diagnostics">
              <dl>
                <dt>Repository</dt>
                <dd>{selectedAddon.repoUrl}</dd>
                <dt>Installed path</dt>
                <dd>{selectedInstalledRecord?.installedPath ?? "Unavailable"}</dd>
                <dt>Install reference</dt>
                <dd>{selectedInstalledRecord?.installRef ?? "Unavailable"}</dd>
              </dl>
              <SettingsActionList
