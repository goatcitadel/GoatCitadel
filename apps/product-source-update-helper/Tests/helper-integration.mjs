...
test("native helper restores the exact prior tree when post-restart smoke fails", { timeout: 45_000 }, async (t) => {
  ...
  t.after(async () => {
    try {
      ...
      // Best-effort cleanup: the helper or test teardown may already have stopped the process.
    }
...
test(
  "native helper resumes the exact request after process death around the commit boundary",
  { timeout: 90_000 },
  async (t) => {
    if (process.platform !== "win32") return t.skip("Windows helper proof runs only on Windows.");
    await fs.access(helperPath);
    for (const phase of ["patch_applied", "patch_committed"]) {
      ...
      t.after(async () => {
        try {
          ...
          // Best-effort cleanup: a crash-boundary phase can end before the pid file is durable.
        }
...
async function waitForJournalEvent(journalPath, eventType, helper) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      // The journal is created asynchronously; keep polling until it exists or the helper exits.
    }
    if (helper.child.exitCode !== null) {
      ...
      try {
        ...
        // Preserve the primary early-exit diagnostic when no journal was written.
      }
...
async function createSourceFixture(sourceRoot) {
  ...
  await fs.writeFile(
    path.join(sourceRoot, "apps", "gateway", "package.json"),
    JSON.stringify({ name: "@goatcitadel/gateway" }),
    "utf8",
  );
  await fs.writeFile(
    path.join(sourceRoot, "apps", "mission-control-next", "package.json"),
    JSON.stringify({ name: "@goatcitadel/mission-control-next" }),
    "utf8",
  );
  await fs.writeFile(path.join(sourceRoot, "docs", "1_0_CONTRACT.md"), "# Contract\n", "utf8");
  await fs.writeFile(
    path.join(sourceRoot, "apps", "gateway", "src", "native-helper-fixture.ts"),
    "export const nativeHelperFixture = 1;\n",
    "utf8",
  );
  git(sourceRoot, ["init"]);
