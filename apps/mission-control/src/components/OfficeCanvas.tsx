...
export function buildCircularLayout(agents: OfficeDeskAgent[]): SeatLayout[] {
  if (agents.length === 0) {
    return [];
  }
...
export function buildZoneAnchors(layout: SeatLayout[]): Map<OfficeZoneId, [number, number, number]> {
...
export function getAgentShapeProfile(seat: SeatLayout): AgentShapeProfile {
...
export function statusHoloColor(status: OfficeDeskAgent["status"], risk: OfficeDeskAgent["risk"]): string {
...
export function activityLabel(activityState: OfficeActivityState): string {
...
export function attentionLabel(attentionLevel: OfficeAttentionLevel | undefined): string {
...
export function attentionChipKind(attentionLevel: OfficeAttentionLevel | undefined): "active" | "approval" | "blocked" {
  if (attentionLevel === "priority") {
    return "blocked";
  }
  if (attentionLevel === "watch") {
...
export function activityChipKind(
  activityState: OfficeActivityState,
  risk: OfficeDeskAgent["risk"],
...
export function operatorPresetPalette(preset: OperatorPreset): {
  body: string;
  skin: string;
  accent: string;
} {
...
export function motionScalarForMode(mode: OfficeMotionMode): number {
  if (mode === "cinematic") {
    return 1;
  }
  if (mode === "balanced") {
...
export function zoneWorkloadColor(telemetry: OfficeZoneSceneTelemetry | undefined, zoneId: OfficeZoneId): string {
