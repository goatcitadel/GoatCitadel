...
import { canonicalJsonString, createRemoteWorkerNativeCapacityDelivery, REMOTE_WORKER_NATIVE_CAPACITY_PAGE_EXCHANGE_SCHEMA,
...
import { uploadWorkerNativeCapacityDelivery, deliverWorkerNativeCapacityOnConnection } from "./worker-native-capacity-page-client.js";
...
function fixture() {
  const f = nativeCapacityCompositionFixture({ extra: 160 });
  const delivery = createRemoteWorkerNativeCapacityDelivery(canonicalJsonString({ layout: f.layout, window: f.window, source: f.source }), f.history, f.window.nonce);
  const json = canonicalJsonString(delivery), bytes = Buffer.from(json, "utf8"), controller = new AbortController();
  let lease: LeaseBinding = { registryWorkspaceId: f.history.registryWorkspaceId, assignmentId: f.history.assignmentId,
    assignmentGeneration: f.history.assignmentGeneration, leaseRevision: f.history.leaseRevision, leaseToken: "synthetic-test-lease" };
...
describe("protected native capacity page client", () => {
  it("composes the durable capture ledger with page retry and a confirmed quarantine decision", async () => {
    ...
    const input = { context: f.input.context, currentLease: f.input.currentLease, signal: f.input.signal,
      history: delivery.history, captureNonce: delivery.window.nonce, state: createInMemoryWorkerDurableState(),
      assertCurrent: async () => undefined,
      capture: vi.fn(async () => canonicalJsonString({ layout: delivery.layout, window: delivery.window, source: delivery.source })) };
    await expect(deliverWorkerNativeCapacityOnConnection(input)).rejects.toThrow(/response lost/u);
    const result = await deliverWorkerNativeCapacityOnConnection(input);
    ...
    expect(await deliverWorkerNativeCapacityOnConnection(input)).toEqual(result);
  ...
  it("renews request authority without changing the original capture identity", async () => {
    ...
    expect(calls.at(-1)![0].payload.leaseRevision).toBeGreaterThan(f.input.delivery.history.leaseRevision);
  });
