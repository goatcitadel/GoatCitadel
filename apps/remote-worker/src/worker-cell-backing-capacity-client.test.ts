...
import { normalizeRemoteWorkerCellBackingCapacityExchange, readRemoteWorkerCellBackingCapacityObservation,
  REMOTE_WORKER_CELL_BACKING_CAPACITY_EXCHANGE_SCHEMA_VERSION, REMOTE_WORKER_CELL_CAPACITY_SUCCESS_RECEIPT } from "@goatcitadel/contracts";
...
import { exchangeWorkerCellBackingCapacity, projectWorkerCellBackingCapacityResponse, observeAndRecordWorkerCellBackingCapacityOnConnection } from "./worker-cell-backing-capacity-client.js";
...
function fixture() {
  ...
  const submission = { kind: "cell.backing_capacity.observation" as const, expectedRevision: 0, observationHex: observation.observationHex,
  ...
  return { lease: native.lease, history, observation, submission, result,
    body: { schemaVersion: "goatcitadel.remote-worker-assignment-execution-response.v1", operation: "assignment.settlement.submit",
      disposition: "cell_backing_capacity_recorded", registryWorkspaceId: history.registryWorkspaceId, cellBackingCapacity: result } };
}
describe("worker backing capacity exchange client", () => {
  it("binds exact native receipts to the protected current lease without including its secret in idempotency", async () => {
    ...
    expect(await exchangeWorkerCellBackingCapacity({ client: {}, credential: {} } as RouteContext, f.lease, f.submission, signal)).toEqual(f.result);
    ...
    expect(request.idempotencyKey).toMatch(/^cell-backing-capacity:[0-9a-f]{64}$/u);
  ...
  it("permits an empty current snapshot and replay under a renewed lease", () => {
    const f = fixture();
    expect(projectWorkerCellBackingCapacityResponse({ ...f.body, disposition: "cell_backing_capacity_snapshot",
      cellBackingCapacity: { ...f.result, record: null } }, f.lease, { kind: "cell.backing_capacity.snapshot" }).record).toBeNull();
    expect(projectWorkerCellBackingCapacityResponse({ ...f.body, cellBackingCapacity: { ...f.result, history: { ...f.history, leaseRevision: 3 } } },
  ...
  it("composes native observation, durable delivery and the RPC client across lease renewal", async () => {
    ...
    vi.mocked(callProtectedRoute).mockImplementation(async (request) => {
      ...
      const submitted = payload.submission.kind === "cell.backing_capacity.observation";
      return { status: 200, body: { ...f.body, disposition: submitted ? "cell_backing_capacity_recorded" : "cell_backing_capacity_snapshot",
        cellBackingCapacity: { ...f.result, history: { ...f.history, leaseRevision: payload.leaseRevision },
    ...
    const result = await observeAndRecordWorkerCellBackingCapacityOnConnection({ scope: {
  ...
  it.each(["scope", "assignment", "generation", "lease", "missing", "revision", "changed", "receipt", "history", "disposition"])(
    "rejects a %s response mismatch", (failure) => {
      ...
      if (failure === "disposition") body.disposition = "cell_backing_capacity_snapshot";
      const result = { ...body.cellBackingCapacity, history: { ...body.cellBackingCapacity.history }, record: body.cellBackingCapacity.record && { ...body.cellBackingCapacity.record } };
      ...
      if (failure === "changed") { const bytes = Buffer.from(result.record!.observationHex, "hex"); bytes.writeBigUInt64LE(bytes.readBigUInt64LE(384) - 1n, 384); result.record!.observationHex = bytes.toString("hex"); }
      if (failure === "history") result.history.mountedWorkspaceRecords = [];
      expect(() => projectWorkerCellBackingCapacityResponse({ ...body, cellBackingCapacity: result }, f.lease, f.submission)).toThrow();
    });
});
