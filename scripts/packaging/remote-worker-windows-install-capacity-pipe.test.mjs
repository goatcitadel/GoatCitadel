...
test("installation reservation pipe binds fresh authority and poisons uncertain exchanges", { skip: process.platform !== "win32", timeout: 240000 }, t => {
  const output = fs.mkdtempSync(path.join(os.tmpdir(), "Goat Install Capacity Pipe "));
  t.diagnostic(`Retained installation reservation pipe evidence: ${output}`);
  const fixture = "apps/remote-worker-windows-cell-native/tests/cell_install_capacity_pipe_test.cpp";
  ...
  for (const asan of [false, true]) {
    ...
    assert.equal(receipt.passed, true); assert.ok(receipt.checks >= 30);
  ...
  fs.writeFileSync(path.join(output, "acceptance.json"), JSON.stringify({ outcomes, sourceManifest: snapshot.sourceManifest,
    boundary: "Real private pipes with controlled current-authority callbacks. No installed endpoint custody, Gateway reservation, copying or volume operation." }, null, 2), { flag: "wx" });
});
