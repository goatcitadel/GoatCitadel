...
const IDENTITY_LOGO_FILES = ["StoreLogo.png", "Square44x44Logo.png", "Square150x150Logo.png"];
...
// The manifest's <Logo>/VisualElements paths resolve inside the package, not the
// external location, so the tiles (generated by `pnpm brand:derive`) ship in it.
function copyIdentityLogos() {
  const sourceDir = path.join(repoRoot, "apps", "mission-control-windows", "Assets");
  const targetDir = path.join(contentDir, "Assets");
  fs.mkdirSync(targetDir, { recursive: true });
  for (const name of IDENTITY_LOGO_FILES) {
    const source = path.join(sourceDir, name);
    if (!fs.existsSync(source)) {
      throw new Error(`Missing identity logo ${source}. Run: pnpm brand:derive`);
    }
