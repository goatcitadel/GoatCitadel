...
<#
.SYNOPSIS
  Observe client-visible signer metadata after the exact GOATBOX protocol failure.
.DESCRIPTION
  Default is a read-only preflight. -StartOnce starts only the installed signer
  once, records SCM transitions and process/token inspection for twelve seconds,
  and saves a report. No protocol connection is made; the broker stays stopped.
  The initial states must match the observed signer 1066/6 and broker 1066/3.
  Queries run as the administrator observer, not the native client.
...
namespace GoatCitadel.SignerClientBoundaryDiagnostic {
  ...
  public sealed class Status {
    public string name;
    public uint type, state, processId, flags, win32ExitCode, serviceExitCode, checkpoint, waitHint, configurationBytesNeeded;
  ...
  public sealed class Report {
    ...
    public string observerContext = "administrator; query results do not prove access from the native client token";
  ...
  public static class Reader {
    ...
    static readonly string[] Hashes = {
      "be670dfd61f42bcf01b182ccdac4e3c3551c3fe3cd08290d08210a9e60c08846",
      "069241c7653b5f314a16f9c6105bd615698ffd458853762dedb2d116a5cf14d8",
    ...
    static Status AssertInitialConfig(IntPtr service, string name) {
      ...
      if (name == Broker && (status.type != 16 || status.state != 1 || status.pid != 0 || status.win32Exit != 1066 || status.serviceExit != 3))
        throw new InvalidOperationException("The broker must already be stopped with no PID.");
      if (name == Signer && (status.type != 16 || status.state != 1 || status.pid != 0 || status.win32Exit != 1066 || status.serviceExit != 6))
        throw new InvalidOperationException("Signer must match the observed stopped protocol failure (1066/6). Current state=" +
      ...
      try {
        ...
        initial.configurationBytesNeeded = needed;
    ...
    public static Report Run(bool startOnce) {
      ...
      try {
        ...
        for (int i=0; i<Names.Length; ++i) {
          services[i] = OpenServiceW(manager, Names[i], (uint)(5 | (startOnce && i == 0 ? 16 : 0)));
        ...
        using (var ready = new CountdownEvent(Names.Length)) {
          try {
            ...
            result.startReturnedSuccess = StartServiceW(services[0], 0, IntPtr.Zero);
        ...
        for (int i=0; i<Names.Length; ++i) {
          ...
          if (i == 0 && found[i] == null) result.errors.Add("No process sample captured for " + Names[i]);
        }
...
if ($SelfTest) {
  [GoatCitadel.SignerClientBoundaryDiagnostic.Reader]::SelfTest() | ConvertTo-Json -Depth 5
  return
}

Write-Host 'Verifying the installed broker-inspection-fix binaries and current service configurations...'
if (-not $StartOnce) {
  [GoatCitadel.SignerClientBoundaryDiagnostic.Reader]::Run($false) | ConvertTo-Json -Depth 6
...
$output = Join-Path $evidenceRoot ('signer-client-boundary-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff') + '.json')
$file = New-Object IO.FileStream($output, [IO.FileMode]::CreateNew, [IO.FileAccess]::Write, [IO.FileShare]::Read)
try {
  Write-Host 'Starting only the verified signer once; recording status transitions and signer inspection for 12 seconds...'
  $report = [GoatCitadel.SignerClientBoundaryDiagnostic.Reader]::Run($true)
