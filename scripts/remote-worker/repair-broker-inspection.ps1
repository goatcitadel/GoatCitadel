...
<#
.SYNOPSIS
  Update the two pinned GOATBOX broker/signer images while preserving installed state.
.DESCRIPTION
  ...
  only those two existing files through held handles, preserving file identity
  ...
  this is not an atomic transaction across two executables. Never remove state.
...
if (-not $output.StartsWith('C:\worker-evidence\broker-inspection-', [StringComparison]::OrdinalIgnoreCase) -or
    [IO.Path]::GetDirectoryName($output) -cne 'C:\worker-evidence' -or (Test-Path -LiteralPath $output)) {
  throw 'Use a new broker-inspection-* output folder directly under C:\worker-evidence.'
}
...
$report = [ordered]@{ schema='goatcitadel.goatbox.broker-inspection-update/1'; mode=$(if ($Apply) { 'apply' } else { 'preflight' });
...
$old = @{
  'GoatCitadelRemoteWorkerProvisioner.exe'='b3b056ef523b57bce4e6f3aec20ceefcd69e524961bc0d1167c1184fa6235381';
  'GoatCitadelRemoteWorkerProvisionerAvailability.exe'='48402f8212e1e0a57d1d36ff546ab781b1ed0731a89470d6628e3c5fec0196b0';
...
try {
  ...
  foreach ($name in @($script:SignerExecutableName, $script:BrokerExecutableName, $script:ClientExecutableName)) {
    ...
    if ($isClient -and $nextHash -ne $old[$name]) { throw 'This repair requires the unchanged client executable.' }
    $sddl = $(if ($name -ceq $script:BrokerExecutableName) { $script:BrokerImageSddl } else { $script:SignerImageSddl })
    $image = $imageType::Open((Join-Path $paths.BinDirectory $name), $old[$name], $sddl, (-not $isClient))
    $leases.Add($image)
    if (-not $isClient) { $images.Add([pscustomobject]@{ name=$name; lease=$image; bytes=$nextBytes; nextHash=$nextHash }) }
    $report.images += [ordered]@{ name=$name; path=$image.Path; identity=$image.Identity; originalSha256=$image.OriginalSha256;
      replacementSha256=$nextHash; securitySddl=$image.SecuritySddl; backup=$(if ($isClient) { $null } else { $name + '.before' }) }
  }
  Assert-ServiceConfiguration
  if ($Apply) {
  ...
  Save-Receipt 'broker-inspection-update-evidence.json'
}
