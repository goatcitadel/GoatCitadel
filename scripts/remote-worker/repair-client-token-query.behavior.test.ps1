...
try {
  ...
  foreach ($name in $names) {
    ...
    [IO.File]::WriteAllText($next,($('replacement ')+$name))
...
'@
  ...
  if ($Case -eq 'rollback') { $source=$source.Replace('$image.lease.Replace($image.bytes, $image.nextHash)',"if (`$image.name -like '*Client.exe') { throw 'injected third-image failure' }; `$image.lease.Replace(`$image.bytes, `$image.nextHash)") }
  $copy=Join-Path $root 'repair-test-copy.ps1'
  [IO.File]::WriteAllText($copy,$source)
  $output=Join-Path $root 'worker-evidence\client-token-query-test'
  ...
  foreach ($name in $names) {
    ...
    if ($Case -eq 'apply') { $expected=(Get-FileHash -LiteralPath (Join-Path $package ('app\provisioner\'+$name))).Hash }
  ...
  if ($Case -eq 'rollback' -and ($report.refusals -join '') -notmatch 'injected third-image failure') { throw ($text|Out-String) }
