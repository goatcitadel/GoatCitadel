...
if (
  ...
  throw new Error(
    "Usage: node agent-comparison-openclaw-workflow-conformance.mjs ABSOLUTE_OPENCLAW_CHECKOUT ABSOLUTE_NODE NEW_ABSOLUTE_DIRECTORY",
  );
...
const transportProfile = {
  ...
  pricing: {
    ...
    sourceSha256: sha256("synthetic OpenClaw workflow fixture"),
  },
};
...
try {
  result = await runNativeComparison({
    ...
    fetchUpstream: async (_url, init) => {
      ...
      let tool;
      if (capture && has("skill_workshop") && once("capture:create"))
        tool = {
          name: "skill_workshop",
          arguments: {
            action: "create",
            name: "release-note",
            description: "Prepare verified release notes from supplied changes, evidence, and unsupported claims.",
            proposal_content: skill,
            goal: "Reuse the verified release-note workflow from this session.",
            evidence:
              "The source-release.md output was checked against input/source-changes.json in the preceding turn.",
          },
        };
      else if (reuse && has("skill_workshop") && once("reuse:skill"))
        tool = { name: "skill_workshop", arguments: { action: "read", skill_name: "release-note" } };
      else if (!capture && has("read") && once(`${phase}:input`))
        tool = { name: "read", arguments: { path: reuse ? "input/changes.json" : "input/source-changes.json" } };
      else if (!capture && has("write") && once(`${phase}:write`)) {
      ...
      } else if (!capture && has("read") && once(`${phase}:verify`))
        tool = { name: "read", arguments: { path: reuse ? "release.md" : "source-release.md" } };
      const message = tool
        ? {
            role: "assistant",
            content: null,
