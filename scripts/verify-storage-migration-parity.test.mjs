...
test("current registries and checked-in manifest cover every migration exactly", async () => {
  ...
  assert.equal(sqlite.migrations.length, 196);
  assert.deepEqual([sqlite.firstVersion, sqlite.lastVersion], [1, 196]);
  assert.equal(postgres.migrations.length, 140);
  assert.deepEqual([postgres.firstVersion, postgres.lastVersion], [1, 140]);
  assert.equal(
  ...
  assert.deepEqual(
    ...
    [2, 119, 120, 121, 122, 134, 135, 136, 137, 138, 140],
  );
