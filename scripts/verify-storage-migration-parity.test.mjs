...
test("current registries and checked-in manifest cover every migration exactly", async () => {
  ...
  assert.equal(sqlite.migrations.length, 187);
  assert.deepEqual([sqlite.firstVersion, sqlite.lastVersion], [1, 187]);
  assert.equal(postgres.migrations.length, 130);
  assert.deepEqual([postgres.firstVersion, postgres.lastVersion], [1, 130]);
  assert.equal(
