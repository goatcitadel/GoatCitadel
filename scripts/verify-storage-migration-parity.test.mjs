...
test("current registries and checked-in manifest cover every migration exactly", async () => {
  ...
  assert.equal(sqlite.migrations.length, 194);
  assert.deepEqual([sqlite.firstVersion, sqlite.lastVersion], [1, 194]);
  assert.equal(postgres.migrations.length, 137);
  assert.deepEqual([postgres.firstVersion, postgres.lastVersion], [1, 137]);
  assert.equal(
  ...
  assert.deepEqual(
    ...
    [2, 119, 120, 121, 122, 134, 135, 136, 137],
  );
