...
import { REMOTE_WORKER_CELL_CAPACITY_SUCCESS_RECEIPT, readRemoteWorkerCellCapacityObservation } from "@goatcitadel/contracts";
...
export const verifyCellCapacityExchange: typeof verifyCellProvisioningExchange = (db, key, token, fence, revoke) => {
  verifyCellProvisioningExchange(db, key, token, fence, revoke, (current, history) => {
    ...
    const snapshot = { ...current, submission: { kind: "cell.capacity.snapshot" as const } };
    assert.equal(repo.exchangeWithAssignment(snapshot).record, null);
    const bytes = capacityObservationFixture(history);
    const write = { ...current, submission: { kind: "cell.capacity.observation" as const, expectedRevision: 0,
      observationHex: bytes.toString("hex"), nativeReceiptHex: REMOTE_WORKER_CELL_CAPACITY_SUCCESS_RECEIPT } };
    const count = () => db.prepare("SELECT COUNT(*) AS count FROM remote_worker_cell_capacity_observations WHERE assignment_id = @assignmentId")
    ...
    assert.equal(readRemoteWorkerCellCapacityObservation(first.record!.observationHex, first.history).logicalFileBytes, 9000);
    assert.deepEqual(new RemoteWorkerCellCapacityRepository(db).exchangeWithAssignment(write), first);
    assert.equal(count(), 1);
    bytes.writeBigUInt64LE(10000n, 328);
    ...
    const next = { ...write, submission: { ...write.submission, expectedRevision: 1,
      observationHex: capacityObservationFixture(history, 0x72).toString("hex") } };
    ...
    db.prepare = (sql) => {
      ...
      if (/^INSERT INTO remote_worker_cell_capacity_observations/u.test(sql)) {
    ...
    assert.throws(() => db.prepare("UPDATE remote_worker_cell_capacity_observations SET revision = revision WHERE assignment_id = @assignmentId").run(bindings), /immutable/u);
    assert.throws(() => db.prepare("DELETE FROM remote_worker_cell_capacity_observations WHERE assignment_id = @assignmentId").run(bindings), /retained/u);
    ...
    const afterSetup = { ...next, submission: { ...next.submission, expectedRevision: 2,
      observationHex: capacityObservationFixture(history, 0x73).toString("hex") } };
