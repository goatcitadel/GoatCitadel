...
describe("Storage.deleteChatSessionData", () => {
  it("removes session-scoped chat data and preserves other sessions", () => {
    ...
    assert.deepEqual(result.cleanupRelPaths.sort(), [
    ...
    assert.equal(countRows(storage, "chat_generated_artifacts", "session_id = 'sess-1'"), 0);
    assert.equal(countRows(storage, "chat_thread_knowledge_attachments", "session_id = 'sess-1'"), 0);
    assert.equal(countRows(storage, "knowledge_documents", "namespace = 'chat-session:sess-1:knowledge'"), 0);
    assert.equal(countKnowledgeChunksByNamespace(storage, "chat-session:sess-1:knowledge"), 0);
  ...
  it("deletes a complete side-chat tree child-first while preserving an unrelated session", () => {
    const storage = createStorage();
    seedChatSession(storage, "tree-parent");
    seedChatSession(storage, "tree-child");
    seedChatSession(storage, "tree-unrelated");
    storage.chatSideChats.upsert(
      {
        sideChatId: "side-tree-parent",
        parentSessionId: "tree-parent",
        childSessionId: "tree-child",
        workspaceId: "default",
    ...
    const result = storage.deleteChatSessionData("tree-parent");

    assert.equal(result.deleted, true);
    assert.deepEqual(result.cleanupRelPaths.sort(), [
      "chat/default/artifacts/tree-child-artifact.txt",
      "chat/default/artifacts/tree-parent-artifact.txt",
      "chat/default/attachments/tree-child-thumb.png",
      "chat/default/attachments/tree-child.txt",
      "chat/default/attachments/tree-parent-thumb.png",
      "chat/default/attachments/tree-parent.txt",
    ]);
    for (const sessionId of ["tree-parent", "tree-child"]) {
      assert.equal(countRows(storage, "sessions", `session_id = '${sessionId}'`), 0);
      assert.equal(countRows(storage, "chat_session_meta", `session_id = '${sessionId}'`), 0);
      assert.equal(
        countRows(
          storage,
          "chat_session_control_grants",
          `session_id = '${sessionId}' AND is_current = 0 AND lease_state = 'deleted'`,
        ),
        1,
      );
      assert.equal(
        countRows(
          ...
          `session_id = '${sessionId}' AND reason_code = 'session_deleted'`,
        ),
        1,
      );
    }
    assert.equal(countRows(storage, "sessions", "session_id = 'tree-unrelated'"), 1);
    assert.equal(countRows(storage, "chat_session_meta", "session_id = 'tree-unrelated'"), 1);
  });
});
...
function seedChatSession(storage: Storage, sessionId: string): void {
  ...
  storage.chatGeneratedArtifacts.create({
    artifactId: `generated-${sessionId}`,
    sessionId,
    workspaceId: "default",
    turnId: `turn-${sessionId}`,
  ...
  const knowledgeNamespace = `chat-session:${sessionId}:knowledge`;
  const document = storage.knowledge.createDocument(
    {
      namespace: knowledgeNamespace,
      sourceType: "file",
      sourceRef: `${sessionId}.txt`,
      title: `${sessionId} attachment`,
      metadata: {
        sessionId,
      },
    },
  ...
  storage.knowledge.appendChunks(
    ...
    [
      {
        content: `knowledge for ${sessionId}`,
      },
    ],
    now,
  );
  storage.chatThreadKnowledgeAttachments.create({
    attachmentId: `thread-knowledge-${sessionId}`,
    sessionId,
    sourceType: "file",
    sourceRef: `${sessionId}.txt`,
    title: `${sessionId} attachment`,
    ...
    chunkCount: 1,
    namespace: knowledgeNamespace,
    chatAttachmentId: `attachment-${sessionId}`,
...
function countKnowledgeChunksByNamespace(storage: Storage, namespace: string): number {
  const row = storage.db
    .prepare(
      `
    SELECT COUNT(1) AS count
