...
export function renderCreateTable(table: SqliteSchemaTableBlueprint): string {
  const primaryKeys = table.columns
    .filter((column) => column.primaryKeyPosition > 0)
    .sort((left, right) => left.primaryKeyPosition - right.primaryKeyPosition)
    .map((column) => column.name);
  const columnDefs = table.columns.map((column) => renderColumn(table, column.name, primaryKeys));
  const foreignKeyDefs = groupForeignKeys(table).map((group) => renderForeignKey(group));
...
export function renderColumn(table: SqliteSchemaTableBlueprint, columnName: string, primaryKeys: string[]): string {
...
export function renderForeignKey(group: SqliteSchemaTableBlueprint["foreignKeys"]): string {
  const first = group[0];
  if (!first) {
  ...
  const clauses = [`FOREIGN KEY (${from}) REFERENCES ${first.referencedTable}(${to})`];
