...
const TABLE = "remote_worker_cell_backing_capacity_observations";
...
function tableSql(postgres: boolean): string {
  ...
  return `CREATE TABLE IF NOT EXISTS ${TABLE} (
    ...
    observation_hex TEXT NOT NULL CHECK (${hex("observation_hex", 848)}),
...
/** Additive host evidence. Rollback disables the producer and retains this
 * history; it never drops measurements or rewrites mounted observation rows. */
export const REMOTE_WORKER_CELL_BACKING_CAPACITY_SQLITE_SQL = `${tableSql(false)}
  CREATE TRIGGER trg_cell_backing_capacity_observation_insert BEFORE INSERT ON ${TABLE}
  WHEN NOT (${AUTHORITY(REMOTE_WORKER_CAPACITY_SQLITE_CLOCK)}) OR NOT (${ORDER})
  BEGIN SELECT RAISE(ABORT, 'cell backing capacity observation authority or revision changed'); END;
  CREATE TRIGGER trg_cell_backing_capacity_observation_immutable BEFORE UPDATE ON ${TABLE}
  BEGIN SELECT RAISE(ABORT, 'cell backing capacity observation is immutable'); END;
  CREATE TRIGGER trg_cell_backing_capacity_observation_retained BEFORE DELETE ON ${TABLE}
  BEGIN SELECT RAISE(ABORT, 'cell backing capacity observation must be retained'); END;
`;
export function createRemoteWorkerCellBackingCapacitySchema(db: Pick<DatabaseSync, "exec">): void {
  db.exec(REMOTE_WORKER_CELL_BACKING_CAPACITY_SQLITE_SQL);
}
export const REMOTE_WORKER_CELL_BACKING_CAPACITY_POSTGRES_SQL = `${tableSql(true)}
  CREATE OR REPLACE FUNCTION gc_cell_backing_capacity_observation_guard() RETURNS trigger AS $$
  BEGIN
    IF TG_OP <> 'INSERT' THEN RAISE EXCEPTION 'cell backing capacity observation is immutable and must be retained' USING ERRCODE = '23514'; END IF;
    IF NOT (${AUTHORITY(REMOTE_WORKER_CAPACITY_POSTGRES_CLOCK)}) OR NOT (${ORDER}) THEN
      RAISE EXCEPTION 'cell backing capacity observation authority or revision changed' USING ERRCODE = '23514';
    END IF;
    RETURN NEW;
  END;
  $$ LANGUAGE plpgsql;
  CREATE TRIGGER trg_cell_backing_capacity_observation_guard BEFORE INSERT OR UPDATE OR DELETE ON ${TABLE}
    FOR EACH ROW EXECUTE FUNCTION gc_cell_backing_capacity_observation_guard();
`;
