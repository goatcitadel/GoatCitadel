...
describe("chat API origin surface headers", () => {
  ...
  it("wires chat API helpers to deterministic gateway routes", async () => {
    ...
    await chat.preflightChatRoute(sessionId, { content: "route me", mode: "chat" } as never, { originSurface: "chat" });
  ...
  it("streams chat and delegation events and reports error responses", async () => {
    const chat = await import("./chat");
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(sseResponse({ type: "delta", sessionId: "session-1", turnId: "turn-1", delta: "resume" }))
      .mockResolvedValueOnce(
    ...
    await chat.streamRetryChatTurn("session-1", "turn-1", { content: "retry" } as never, (chunk) => chunks.push(chunk));
  ...
  it("covers chat stream failures, delegation stream errors, and FileReader attachment encoding", async () => {
    const chat = await import("./chat");
    const chunks: unknown[] = [];
    let fetchMock = vi
      .fn()
