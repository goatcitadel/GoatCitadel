...
// NOTE: This is a self-contained replica of the kill-tree pattern in
// apps/gateway/src/services/process-tree-killer.ts. policy-engine cannot import
// from apps/gateway (that would be a circular dependency), so the minimal logic
// is duplicated here using only Node builtins. Keep the two in sync if the
// underlying platform behavior changes.
...
/**
 * Kills a spawned child process and its descendants. On Windows this shells out
 * to `taskkill /pid <pid> /T /F` (terminates the whole tree, including
 * SIGTERM-ignoring processes); on POSIX it sends SIGTERM then SIGKILL to the
 * process group when `posixProcessGroup` is set, falling back to the child only.
 */
