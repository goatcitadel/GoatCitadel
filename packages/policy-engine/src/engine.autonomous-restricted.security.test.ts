...
async function evaluate(toolName: string, profile: PermissionProfileRecord, args: Record<string, unknown> = {}) {
  const engine = new ToolPolicyEngine(PERMISSIVE_CONFIG, createStorageStub());
  return await engine.evaluateAccess({
    toolName,
    args,
    agentId: "proactive",
...
describe("autonomous restricted profiles via engine.evaluateAccess", () => {
  it("denies side-effecting tools on scheduled-restricted (policy deny-wins)", async () => {
    const shell = await evaluate("shell.exec", SCHEDULED_RESTRICTED_PROFILE, { command: "echo hi" });
    ...
    const write = await evaluate("fs.write", SCHEDULED_RESTRICTED_PROFILE, { path: "./x", contents: "y" });
    expect(write.allowed).toBe(false);
    expect(write.reasonCodes).toContain("policy_deny");

    const mcp = await evaluate("mcp.invoke", SCHEDULED_RESTRICTED_PROFILE);
    ...
    const schedule = await evaluate("schedule.manage", SCHEDULED_RESTRICTED_PROFILE, { op: "list" });
    expect(schedule.allowed).toBe(false);
    expect(schedule.reasonCodes).toContain("policy_deny");
  });

  it("still allows non-denied read/query tools on scheduled-restricted (narrows, not closes)", async () => {
    ...
    const memoryRead = await evaluate("memory.read", SCHEDULED_RESTRICTED_PROFILE, { key: "k" });
    expect(memoryRead.allowed).toBe(true);
    expect(memoryRead.permissionProfileId).toBe(SCHEDULED_RESTRICTED_PROFILE.profileId);

    const status = await evaluate("session.status", SCHEDULED_RESTRICTED_PROFILE);
    expect(status.allowed).toBe(true);
  });

  it("heartbeat-restricted denies everything outside the read-only allowlist", async () => {
    for (const tool of ["shell.exec", "fs.write", "fs.read", "memory.write", "session.status"]) {
      const evaluation = await evaluate(tool, HEARTBEAT_RESTRICTED_PROFILE, { path: "./x" });
  ...
  it("heartbeat-restricted allows its read-only allowlist (memory.read)", async () => {
    const memoryRead = await evaluate("memory.read", HEARTBEAT_RESTRICTED_PROFILE, { key: "k" });
    expect(memoryRead.allowed).toBe(true);
