...
const CHANNEL_RULES: Record<string, ChannelRule> = {
    ...
    resolveRuntimePosture: (config) =>
      (readConfigString(config, "runtimeMode") ?? "bridge") === "gateway"
        ? {
          ...
          operatorSummary: "Discord gateway mode is normalized as a persistent runtime: the bot stays online, paired DMs can route inbound, and allowlisted guild traffic can be policy-gated in-process.",
        }
        : hasAnyConfigured(config, ["botTokenEnv", "botToken", "tokenEnv", "token"])
          ? {
            outboundTransport: "api",
            ...
            operatorSummary: "Discord bridge mode is normalized as a stateless outbound REST bridge. Rich outbound actions work, but inbound routing and online presence are not active.",
          }
          : {
            outboundTransport: "webhook",
            lifecycle: "stateless",
            inboundReadiness: "unsupported",
            operatorSummary: "Discord webhook mode is normalized as a stateless outbound-only path. It can send and delete webhook-authored messages, but it does not accept inbound traffic or maintain presence.",
          },
    chunkingMode: "fallback",
    supportsStreaming: false,
    resolveSupportNotes: (config) =>
  ...
  telegram: {
    ...
    supportsStreaming: false,
    resolveRuntimePosture: (config) => {
      const hasWebhookIngress = hasAnyConfigured(config, ["webhookSecret", "webhookSecretEnv", "secretToken", "secretTokenEnv"]);
      return {
        outboundTransport: "api",
  ...
  "google-chat": {
    ...
    runtimePosture: {
      ...
      lifecycle: "stateless",
      inboundReadiness: "unsupported",
      operatorSummary: "Google Chat is normalized as a stateless outbound webhook destination. Rich card sends work, but inbound routing is not available in the current runtime.",
    },
    supportNotes: [
  ...
  teams: {
    ...
    runtimePosture: {
      ...
      lifecycle: "stateless",
      inboundReadiness: "unsupported",
      operatorSummary: "Teams is normalized as a stateless outbound webhook destination. Adaptive-card sends work, but inbound routing is not available in the current runtime.",
    },
    supportNotes: [
  ...
  signal: {
    ...
    runtimePosture: {
      ...
      lifecycle: "stateless",
      inboundReadiness: "unsupported",
      operatorSummary: "Signal is normalized as a stateless outbound bridge. It can send to configured recipients, but inbound routing is not available in the current runtime.",
    },
  ...
  mattermost: {
    ...
    runtimePosture: {
      ...
      lifecycle: "stateless",
      inboundReadiness: "unsupported",
      operatorSummary: "Mattermost is normalized as a stateless outbound bot bridge. Rich outbound post actions are supported, but inbound routing is not yet wired.",
    },
    supportNotes: [
  ...
  imessage: {
    ...
    runtimePosture: {
      ...
      lifecycle: "stateless",
      inboundReadiness: "unsupported",
      operatorSummary: "iMessage via BlueBubbles is normalized as a stateless local bridge. Outbound sends, replies, reactions, and unsend depend on bridge capabilities, but inbound normalization is not active here.",
    },
    supportNotes: [
  ...
  "nextcloud-talk": {
    ...
    runtimePosture: {
      ...
      lifecycle: "stateless",
      inboundReadiness: "ready",
      operatorSummary: "Nextcloud Talk is normalized as a stateless bot bridge with inbound webhooks. Outbound replies and reactions are available, and inbound events can be routed without a persistent gateway runtime.",
    },
    supportNotes: [
  ...
  zalo: {
    ...
    runtimePosture: {
      ...
      lifecycle: "stateless",
      inboundReadiness: "unsupported",
      operatorSummary: "Zalo is normalized as a stateless outbound bridge. It can send to configured recipients, but inbound routing is not available in the current runtime.",
    },
  ...
  zalouser: {
    ...
    runtimePosture: {
      ...
      lifecycle: "stateless",
      inboundReadiness: "unsupported",
      operatorSummary: "Zalo user bridge delivery is normalized as a stateless outbound path and currently requires URL-backed rich attachments. Inbound routing is not available in the current runtime.",
    },
    supportNotes: [
...
export function describeChannelCapabilities(
  channelKey: string,
  ...
  const inboundModes = uniqueInboundModes([
    ...(rule.resolveInboundModes?.(config) ?? rule.inboundModes ?? ["none"]),
  ]);
  ...
  const runtimePosture = rule.resolveRuntimePosture?.(config)
    ?? rule.runtimePosture
    ?? buildDefaultRuntimePosture(inboundModes);
...
function buildDefaultRuntimePosture(inboundModes: ChannelInboundMode[]): ChannelRuntimePosture {
  ...
  return {
    outboundTransport: "api",
    inboundTransport,
    lifecycle: inboundTransport === "gateway" ? "persistent" : "stateless",
    inboundReadiness: inboundTransport ? "ready" : "unsupported",
    operatorSummary: inboundTransport
      ? `This channel advertises inbound ${inboundTransport} routing in the current runtime.`
      : "This channel is currently normalized as an outbound-only bridge.",
  };
}
