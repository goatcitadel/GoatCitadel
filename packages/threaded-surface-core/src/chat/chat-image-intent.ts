...
const IMAGE_CREATION_WITH_NOUN =
  /\b(?:create|generate|make|render|produce|design|paint|illustrate|sketch)\s+(?:me\s+)?(?:(?:an?|the|some)\s+)?(?:(?!(?:of|about|for|from|with|this|that|these|those)\b)[\w-]+\s+){0,4}(?:image|picture|pic|photo|illustration|drawing|artwork|poster|wallpaper|avatar|logo|icon|sticker|banner|cover|thumbnail|scene|visual)\b/i;
...
const DRAW_SUBJECT_REQUEST = /\b(?:draw|paint|sketch|illustrate)\s+(?:me\s+)?(?:an?\s+|the\s+|some\s+)?\w+/i;
...
const SEARCH_OR_ANALYSIS_REQUEST =
  /\b(?:find|search|look\s+up|download|open|save|analy[sz]e|review|describe|caption|ocr|transcribe|classify)\b/i;
...
export function detectImageGenerationIntent(draft: string): boolean {
  ...
  if (SEARCH_OR_ANALYSIS_REQUEST.test(normalized) && !IMAGE_CREATION_WITH_NOUN.test(normalized)) {
    return false;
  }
  ...
  return DRAW_SUBJECT_REQUEST.test(normalized) && !NON_VISUAL_DRAW_REQUEST.test(normalized);
}
