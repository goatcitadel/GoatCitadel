...
function fixture() {
  ...
  return {
    ...
    plan,
    files,
    read: (hex: unknown = bytes.toString("hex")) => read(hex, plan, f.expectation, resultHex, f.history),
  };
}
describe("native collection identity handoff", () => {
  ...
  it("refuses every changed header byte before exposing selections", () => {
    ...
    // Each of the 108 mutations revalidates the complete retained result; coverage
    // on shared CI runners exceeds Vitest's default five-second budget.
  }, 30_000);
  it("refuses duplicate, missing, directory, truncated and extra identities", () => {
    const f = fixture(),
      duplicate = Buffer.from(f.bytes),
      missing = Buffer.from(f.bytes),
      directory = Buffer.from(f.bytes);
    duplicate.copy(duplicate, 132, 108, 132);
    missing.fill(0, 108, 132);
  ...
  it("enforces both independent per-file and aggregate ceilings", () => {
    ...
    for (const changes of [{ maximumFileBytes: 4 }, { maximumTotalBytes: 4 }])
      expect(() =>
        read(f.bytes.toString("hex"), { ...f.plan, ...changes }, f.expectation, f.resultHex, f.history),
      ).toThrow();
  });
});
