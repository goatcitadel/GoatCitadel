...
export interface CellVolumeFixtureIdentity { readonly ntfsSerial?: bigint; readonly volumeIdHex?: string; readonly mountIdByte?: number }
...
export function formatCheckpointFixture(input: RemoteWorkerCellFormatAnchor, sequence: number,
  previous?: string, ntfsPrevious = "0".repeat(64), identity: CellVolumeFixtureIdentity = {}): string {
  ...
  layout.subarray(480).copy(inner, 48); Buffer.from(identity.volumeIdHex ?? "78563412bc9aef4d8123456789abcdef", "hex").copy(inner, 80);
  ...
  if (sequence === 2) {
    ...
    inner.writeBigUInt64LE(identity.ntfsSerial ?? 0xfedcba9876543210n, 184); inner.writeBigUInt64LE(sectors, 192); inner.writeBigUInt64LE(sectors / 8n, 200);
  }
  rehashVolumeFixture(inner);
  return rehashVolumeFixture(bytes);
}
export function formatExchangeFixture(input: unknown, identity: CellVolumeFixtureIdentity = {}) {
  const exchange = volumeExchangeFixture(input), anchor = remoteWorkerCellProvisioningFormatAnchor(exchange);
  const intent = formatCheckpointFixture(anchor, 1, undefined, undefined, identity);
  const completion = formatCheckpointFixture(anchor, 2, intent.slice(-64), intent.slice(1520, 1584), identity);
