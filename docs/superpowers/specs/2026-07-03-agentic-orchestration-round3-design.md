...
Implementation note (2026-07-03, follow-up session): shipped as designed. Execution rides the normal tool policy path end-to-end — the policy engine authorizes, then dispatches through a `subagentFanout` runtime hook (the `schedule.manage` impure-fulfillment pattern) to a session-scoped executor registry (`chat-subagent-fanout-service.ts`) that the entry/stream turn services register/dispose around each direct turn. Exposure is pinned into the essential tool set for eligible turns (intent scoring can't anticipate fan-out) and excluded for restricted scheduled/heartbeat profiles, mirroring `schedule.manage`'s anti-recursion rule. Children are buffered sends only; per-subtask output excerpted at 3,000 chars (the delegated prior-step handoff bound).

Review hardening (same day, pre-merge): the mandated review pass caught that the executor registry must ALSO ride through `buildChatTurnRuntimeHost` and the `createChatTurnRuntimeHost` whitelist composer — an optional host member dropped there is silently `undefined` in production with no compiler error while fixture-based tests stay green; regression tests now build the host through the real composition. Additionally, `agent.fanout` is held to the expensive-tool remaining-budget floor (`minimumRemainingBudgetForToolStart`) so a turn near its deadline cannot start a 3-child fan-out; deliberately not via `isExpensiveChatTool` to avoid opting into web-scoped browser budget extension. Deferred with evidence: `toolRunCount` weighs a fan-out as one run of `maxToolRunsPerTurn` despite ≤3 spawned children — re-weighting needs its own budget-semantics pass. Post-PR review (Codex P2): executor registration itself is now gated by `shouldRegisterSubagentFanoutExecutor` (cowork/code + subagentPolicy ≠ off), so a delegated child session never holds a live executor — the no-recursion guarantee no longer rests on schema exposure alone.
