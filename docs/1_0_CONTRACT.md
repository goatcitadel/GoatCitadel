...
- `MemoryLifecycleService` is the operator-facing memory lifecycle owner for context composition, explicit recall, trace-derived memory proposals, recall-quality feedback, learned-memory policy, and memory item list/edit/forget/history.
...
- Chat, Cowork, and Code recall modes are explicit API/UI flows: targeted recall, broad summary recall, and post-compaction resume context. They may return inspectable context packs and feedback records, but they must not be represented as invisible automatic prompt injection.
