...
- A2A v1.0 is the governed external agent-to-agent interoperability boundary at the Gateway. Callable support includes JSON-RPC over HTTP/S, peer-authenticated HTTP+JSON task routes, and peer-configured task push notification delivery when explicitly enabled, configured with peer credentials, and covered by Gateway-owned peer auth, deny-wins policy, durable task bindings, audit, network allowlists, focused service/route tests, and `verify:a2a:full`. Public Agent Card discovery at `/.well-known/agent-card.json` remains disabled by default. Authenticated extended cards are peer-gated, not public discovery. gRPC remains a non-callable capability flag until separately implemented and proven. A2A does not replace GoatCitadel mesh; mesh remains the native coordination fabric for readiness, leases, ownership, replication, failover, and LAN/WAN/tailnet runtime state.
...
- broader A2A transport support beyond callable JSON-RPC over HTTP/S, peer-authenticated HTTP+JSON task routes, and peer-configured task push notification delivery, or public discovery by default, unless the matching Gateway routes, peer auth, durable mapping, audit, docs, and focused tests are present
