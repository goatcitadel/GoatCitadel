...
- Chat turns may carry at most 16 structured `contextRefs`, limited to Gateway-owned `attachment` and `memory_item` records. Raw filesystem paths, URLs, session or task references, and Assembly context are not Chat routed-context inputs. The Gateway resolves the records under the turn's server-owned scope, freezes the effective capability profile and provider/model budget, and rejects caller attempts to widen those bindings. Routed-context v1 requires the frozen subagent policy to be `off`; `ask_when_useful` and `auto_when_useful` reject before source resolution. An admitted turn remains on the direct frozen provider route, and model-orchestration planning, delegated fan-out, and their provider calls are bypassed before any planner or delegated provider call. Routed memory is workspace-only in v1; global memory fails closed until an explicit server-owned capability-profile policy admits it.
...
- Skill imports may validate a portable `goatcitadel.skill-bundle.json` manifest with per-asset hashes and review-only script disposition. This is evidence for provenance and re-review, not a tool grant, add-on launch path, or bypass around capability activation and approvals.
