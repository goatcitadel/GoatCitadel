...
- A2A v1.0 is the governed external agent-to-agent interoperability boundary at the Gateway. Callable support is JSON-RPC over HTTP/S only, requires explicit A2A enablement, configured peer credentials, Gateway-owned peer auth, deny-wins policy, durable task bindings, audit, and network allowlists. Public Agent Card discovery at `/.well-known/agent-card.json` remains disabled by default. gRPC, HTTP+JSON, push notifications, and authenticated extended cards are non-callable capability flags until separately implemented and proven. A2A does not replace GoatCitadel mesh; mesh remains the native coordination fabric for readiness, leases, ownership, replication, failover, and LAN/WAN/tailnet runtime state.
...
- broader A2A transport support beyond callable JSON-RPC over HTTP/S, or public discovery by default, unless the matching Gateway routes, peer auth, durable mapping, audit, docs, and focused tests are present
