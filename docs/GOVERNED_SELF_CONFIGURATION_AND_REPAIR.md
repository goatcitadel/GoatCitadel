...
Within one process, duplicate active submissions may be coalesced using a transient keyed HMAC that is deleted at settlement and never persisted. After immutable settlement, any retry returns the existing secret-free durable receipt without comparing, hashing, or reapplying the retried raw value. An expired, foreign, inactive, or stale request without a matching settlement fails closed, clears the field, and requires a fresh prompt. An active reservation is never automatically reclaimed from a run version alone: duplicate or post-restart claims perform no effect, and database-expired ambiguity quarantines credential resolution until a fresh successful replacement reconciles it. Packaged-process fault injection and explicit rollback-failure recovery evidence remain mandatory before the lane can be called complete.
...
The target named proof lane is `pnpm verify:self-configuration`. Until that lane exists and produces a secret-free artifact containing browser/desktop behavior, live synthetic probes, durable restart fault injection, rollback, and storage/log redaction scans, public claims must say that GoatCitadel has repair foundations rather than self-configuration parity.

## Current Foundation and Implementation Order

Current foundations include provider secret persistence, config generations and rollback recovery, doctor repair checks, daemon repair proposals, typed config/secret/tool errors, approvals/effects, secure provider routes, and durable Chat waiting states. The initial search slice now adds a password field, target-owned credential-acquisition help, a dedicated no-store/no-log/rate-limited loopback submit endpoint, installation-scoped OS-keychain targets, bounded official-provider probes, pre-effect SQLite/PostgreSQL reservation CAS, restart/expiry quarantine, receipt-sealed durable continuation, exact retry replay, and narrow post-answer recovery of a side-effect-free tool history. A generic Chat response carrying a secret is rejected. Policies that still require a separate Ward approval at apply time fail closed with an administrator-intervention message rather than displaying a dead-end password form; binding one-time approval authority through secure submit and a real administrator recovery surface remain follow-on. The broader multi-recipe remediation coordinator, durable reason-specific rollback-failure reconciliation, scoped remote custody, other repair classes, delegated continuation, and packaged live-probe proof remain incomplete, so this is partial rather than generic self-repair parity.

Two current operational restrictions are explicit. Existing installations whose generated network policy predates this slice must add only `api.search.brave.com` and/or `api.parallel.ai` through the Settings/config-generation owner before the credential card can open; the credential flow never widens egress implicitly. Also, an active secure reservation observed after a process crash is not automatically reclaimed. It performs no further effect, expires into quarantine, and requires a fresh replacement prompt from a later turn. Regenerating the original prompt and resuming that exact crashed turn remains required work for durable-continuity parity.
