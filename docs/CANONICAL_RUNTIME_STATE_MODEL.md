...
- Chat delegation (`apps/gateway/src/services/chat-delegation-service.ts`) screens each completed step output with the same scanner, as source `delegated_output`, before a dependent child's first message includes it. A clean output goes in as a fenced data block labelled with its step and role, and the block's end marker carries a digest of its body. A finding withholds that whole output behind a placeholder and records a `diagnostic` task activity with the rule ID and evidence hash.
