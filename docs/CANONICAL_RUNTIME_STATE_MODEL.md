...
- Cowork/orchestration runs are durable-run backed. Each run allocates and leases its own git worktree and reads phase specs from it, but phase agents run as ordinary child Chat turns with the session's normal workspace scope, not inside that worktree. Wave `ownership` paths are checked for overlap when a plan is validated; they are not enforced on tool writes. `orchestration_runs.status`, `currentWaveId`, and `currentPhaseId` describe plan/operator position; `durableRunId`, `executionState`, `worktreePath`, `worktreeStatus`, and `worktreeBaseRef` describe execution truth. Terminal cleanup removes a registered worktree only after git, reading it through its registration, reports no staged, modified, untracked, or ignored files and the removal succeeds; an empty unregistered directory can also be removed. Otherwise the worktree is kept (run event `run.worktree_retained_dirty` or `run.worktree_retained_unverified`), its lease is released, and the orphan reaper leaves it in place until git reports it clean or an unregistered directory is empty.
...
- An orchestration run and its linked durable run end together, and the orchestration run never ends ahead of its durable run for an error. When the durable worker fails an orchestration workflow that threw, the orchestration run follows as soon as that failure commits; if an operator pause or cancel took the lease first, the run is left to that owner. When a durable run ends outside the orchestration lifecycle (an operator cancel through the durable API, a continuation gate, a dead letter, or a failure whose follow-up did not land), a reconciler on the durable worker's recovery pass settles the orchestration run to the durable outcome, with its error text redacted and bounded, and releases its worktree. If a durable run executes for an orchestration run that has already ended, it settles to that run's outcome without executing anything. Worktree allocation, queueing, and cancellation write the run with compare-and-set, so a concurrent cancel is never overwritten and a cancelled run's durable run is never resumed. A blocking `orchestration.run.before` hook fails the run, cancels its paused durable run, releases its worktree, and returns a conflict.
- Ownership durable runs are created paused before orchestration linkage. Recovery fails aged, unlinked created owners with compare-and-set, then cancels unused queued or paused ownership durables for failed or cancelled owners. A setup still within its grace period is left alone.
