...
- Durable execution is the canonical owner for the shipped resumable mission-session Chat flow set: LLM HTTP/SSE send entrypoints, approval wait/resume, linked proactive wakes, durable-linked chat stream resumption, worker restart recovery, retry scheduling, and dead-letter recovery mechanics. Planning, research, delegation, and code-capability turns run inside Chat.
- External writeback sessions remain visible. Integration operator write actions record audit-only durable `external_writeback` evidence envelopes. Local bridge writes, Activepieces triggers, Trello card creation, and Gmail send actions claim idempotency before crossing the external boundary and mark the external-call boundary immediately before provider/local-bridge fetches. Mission Control may enqueue a replay-audit durable run from the external side-effect ledger; describe it as an eligibility/evidence check with Run Detail linkage, not as a guaranteed retry. The replay-safe worker and durable `external_side_effect.replay` workflow can retry only failed-before-boundary or stale claimed-not-sent runs when an allowlisted owning integration reconstructs the original safe payload; unknown external outcomes still require manual reconciliation. Without an owning replay job, the durable workflow records skipped/manual-reconciliation evidence and sends nothing. Activepieces webhook responses may preserve workflow-run id/status/url evidence, and explicit operator-triggered `check_run_status` reads may fetch sanitized run status from the configured Activepieces API. The bridge remains explicit operator-run execution rather than autonomous workflow polling or lifecycle management. Workflow recipe Activepieces template export is read-only operator-import planning evidence; it must not be described as flow creation, webhook execution, or background Activepieces status monitoring.
- The dormant external connector catalog imports the pinned MindStudio Connector Registry snapshot as review data only. Gateway exposes source/service/action metadata, configuration summaries, handler hashes, review state, and capability-proposal staging; it does not execute imported handlers, install upstream SDKs, create integration connections, run OAuth, or cross external side-effect boundaries for those entries. Promote one action at a time through normal capability governance before making it callable.
...
- The former NPU sidecar path is retired from the shipped `1.0` source and installer. Do not add ONNX/NPU acceleration back to default runtime or release packaging without real hardware proof and updated product truth.
...
- Surface-regression and visual-regression derive from the same canonical Mission Control Next release-surface manifest, covering the current `Work / Projects / Library / Ops / Settings` operator navigation, where Work is Chat, while legacy `Work / Observe / Tune` remains release taxonomy only.
...
- Terminal cleanup removes the run worktree, or keeps it when git reports uncommitted work (`run.worktree_retained_dirty`); the hourly orphan reaper skips kept worktrees until they are clean.
...
- `GET /api/v1/integrations/external-connectors/sources`
- `GET /api/v1/integrations/external-connectors/services`
- `GET /api/v1/integrations/external-connectors/services/:sourceId/:serviceId`
- `GET /api/v1/integrations/external-connectors/services/:sourceId/:serviceId/actions/:actionId`
- `PATCH /api/v1/integrations/external-connectors/services/:sourceId/:serviceId/review`
- `PATCH /api/v1/integrations/external-connectors/services/:sourceId/:serviceId/actions/:actionId/review`
- `POST /api/v1/integrations/external-connectors/services/:sourceId/:serviceId/actions/:actionId/stage`
  - Dormant connector catalog review surface. These routes mutate only GoatCitadel review/proposal state; they do not create live connections or execute imported connector code.
