...
- Memory retrieval provenance must stay truth-preserving: lexical/recency scoring is the default, semantic-hint scoring may only use operator-visible memory item metadata such as `retrievalHints`, `semanticTerms`, `tags`, `aliases`, or `keywords`, and vector/embedding search must not be claimed unless a separate retrieval path proves it.
