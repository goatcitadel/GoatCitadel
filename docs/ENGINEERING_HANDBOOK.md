...
- External writeback sessions remain visible. Integration operator write actions record audit-only durable `external_writeback` evidence envelopes. Local bridge writes, Activepieces triggers, Trello card creation, and Gmail send actions claim idempotency before crossing the external boundary and mark the external-call boundary immediately before provider/local-bridge fetches. The replay-safe worker can retry only failed-before-boundary or stale claimed-not-sent runs when the owning integration reconstructs the original safe payload; unknown external outcomes still require manual reconciliation. Activepieces webhook responses may preserve workflow-run id/status/url evidence, but the bridge remains explicit operator-run execution rather than autonomous workflow polling. Workflow recipe Activepieces template export is read-only operator-import planning evidence; it must not be described as flow creation, webhook execution, or live Activepieces status monitoring.
